...
- **Switch sources from the Sessions sidebar.** A named source selector appears when more than one source is registered and handles any registry size without making gateways look like profiles. The adjacent profile rail then shows only that source's agents and remembers the last profile used there; large profile sets condense independently.
