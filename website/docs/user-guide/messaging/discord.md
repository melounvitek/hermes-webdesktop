...
This permits deliberate handoffs and multi-turn collaboration between bots: each bot must explicitly type the next bot's mention on every turn it wants that bot to process, so a reply on its own never re-triggers the peer. When a bot's tagged handoff is longer than one Discord message, the unmentioned continuation chunks it sends within `HERMES_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS` of the tag (same sender, same channel or thread) are batched into the same turn instead of being dropped; this is a proximity window, so an unrelated message from that bot inside it joins the batch. The gateway's bot loop guard remains as a backstop: after 20 bot-authored messages in one channel inside 5 minutes, further bot messages there are dropped for 10 minutes (tunable under `gateway.bot_loop_guard` in `config.yaml`; human messages are never counted). Set `discord.bots_require_inline_mention: false` only for a trusted relay integration that intentionally depends on unmentioned bot messages or reply pings; doing so restores the reply-loop risk.
:::
