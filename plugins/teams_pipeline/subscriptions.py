...
def sync_graph_subscription_record(
    store: TeamsPipelineStore, subscription_payload: dict[str, Any], *, status: str | None = None,
    renewed: bool = False,
) -> dict[str, Any]:
...
def is_managed_subscription(
    store: TeamsPipelineStore, subscription_payload: dict[str, Any], *, expected_client_state_value: str | None,
) -> bool:
...
async def maintain_graph_subscriptions(
    *, client: MicrosoftGraphClient, store: TeamsPipelineStore, renew_within_hours: int = 24, extend_hours: int = 24,
    ...
    for raw in remote_subscriptions:
        ...
        candidate = {
            "subscription_id": subscription_id, "resource": raw.get("resource"),
        ...
        patched = await client.patch_json(
            f"/subscriptions/{subscription_id}", json_body={"expirationDateTime": new_expiration}
        )
    ...
    for subscription_id in store.list_subscriptions():
        if subscription_id not in remote_ids:
            store.upsert_subscription(
                subscription_id, {"status": "missing_remote", "last_seen_missing_remote_at": utc_timestamp()}
            )
