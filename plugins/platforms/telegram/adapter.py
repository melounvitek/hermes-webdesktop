...
from gateway.platforms.base import (
    ...
    cache_image_from_bytes, cache_audio_from_bytes, cache_video_from_bytes, resolve_proxy_url, SUPPORTED_VIDEO_TYPES,
    ...
    async def _cache_inbound_document(self, msg, event: MessageEvent) -> bool:
        ...
        try:
            ...
            if ext in SUPPORTED_VIDEO_TYPES:
                ...
                self._set_cached_media(
                    event, cache_video_from_bytes(bytes(video_bytes), ext=ext), SUPPORTED_VIDEO_TYPES[ext], MessageType.VIDEO,
            ...
            from gateway.platforms.base import cache_media_bytes
            cached = cache_media_bytes(raw_bytes, filename=original_filename or f"document{ext or '.bin'}", mime_type=doc_mime)
            if cached is None:
    ...
    async def _handle_media_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if msg.photo:
            try:
                ...
                self._set_cached_media(
                    event, cache_image_from_bytes(bytes(image_bytes), ext=ext), f"image/{ext.lstrip('.')}", event.message_type,
