...
class WeComMediaMixin:
    ...
    async def _cache_media(self, kind: str, media: Dict[str, Any]) -> Optional[Tuple[str, str]]:
        ...
        if media.get("base64"):
            ...
            return self._store_media(kind, raw, self._detect_image_ext(raw), "", filename, mimetypes.guess_type(filename)[0] or "application/octet-stream", "")
        ...
        return self._store_media(kind, raw, ext, content_type, self._guess_filename(url, headers.get("content-disposition"), content_type), content_type, f" from {url}")

    def _store_media(self, kind, raw, ext, image_mime, filename, doc_mime, origin) -> Optional[Tuple[str, str]]:
        """Cache bytes as an image (``kind == "image"``) or a document; returns (path, mime)."""
        if kind != "image":
            return cache_document_from_bytes(raw, filename), doc_mime
        try:
            return cache_image_from_bytes(raw, ext), image_mime or self._mime_for_ext(ext, fallback="image/jpeg")
