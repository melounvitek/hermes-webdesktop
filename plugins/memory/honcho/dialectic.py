...
class DialecticMixin:
    ...
    # A thread older than timeout × multiplier is treated as dead so a hung Honcho
    # call can't block future fires indefinitely.
    _STALE_THREAD_MULTIPLIER = 2.0
    # A pending result fired more than cadence × multiplier turns ago is discarded on
    # read, so a stale conversational pivot isn't injected after trivial-prompt turns.
    _STALE_RESULT_MULTIPLIER = 2
    # Ceiling for the empty-streak backoff (× base cadence).
    _BACKOFF_MAX = 8
    ...
    def _effective_cadence(self) -> int:
        ...
        return min(self._dialectic_cadence + self._dialectic_empty_streak, self._dialectic_cadence * self._BACKOFF_MAX)
    ...
    def _spawn_dialectic(
        self, query: str, *, thread_name: str, fired_at: int, log_label: str,
        use_query_rewrite: bool = True,
    ) -> threading.Thread:
        """Start a background dialectic run that publishes into the pending-result slot.

        Only a non-empty result advances ``_last_dialectic_turn`` (so empty returns
        retry next turn) and resets the empty streak; failures widen the backoff.
        """
        def _run() -> None:
            try:
    ...
    def _build_dialectic_prompt(self, pass_idx: int, prior_results: list[str], is_cold: bool) -> str:
        ...
        if pass_idx == 0:
            ...
            return (
                "Given what's been discussed in this session so far, what context about this user is most "
        ...
        passes = [prior_results[i] if len(prior_results) > i else "(empty)" for i in (0, 1)]
        return (
    ...
    def _signal_sufficient(result: str) -> bool:
        ...
        if not result or len(result.strip()) < 100:
            return False
        structured = "\n" in result and bool(
            "##" in result or "•" in result or re.search(r"^(?:[*-] |\s*\d+\. )", result, re.MULTILINE)
        )
    ...
    def _run_dialectic_depth(self, query: str, *, use_query_rewrite: bool = True) -> str:
        ...
        for i in range(self._dialectic_depth):
            ...
            if prior_results:
                ...
                prompt = rewritten_query or self._build_dialectic_prompt(0, prior_results, is_cold)
            ...
            result = self._manager.dialectic_query(
                self._session_key, prompt, reasoning_level=level, peer="user",
            )
