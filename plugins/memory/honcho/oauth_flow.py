...
# Loopback redirect registered for the Hermes OAuth client. IP-literal so the
# browser can't resolve the advertised host to ::1 and miss the IPv4 bind.
LOOPBACK_HOST = "127.0.0.1"
LOOPBACK_PORT = 8765
LOOPBACK_REDIRECT_URI = f"http://{LOOPBACK_HOST}:{LOOPBACK_PORT}/callback"

# Pending authorizations are keyed by the CSRF ``state`` so a forged callback
# can't complete a grant; stale entries are swept after this TTL.
...
# One OAuth client for every surface (consent branding varies via ``source``), so
# there is a single grant identity to refresh — no clientId/refresh-token desync.
_DEFAULT_CLIENT_ID = "hermes-agent"


def _display_config_path(path: object) -> str:
    """Home-relative display string for the consent screen (never the write path).
    A path outside ``$HOME`` yields the bare filename rather than leaking it."""
    p = Path(str(path))
    try:
...
def resolve_endpoints(environment: str | None = None, base_url: str | None = None) -> OAuthEndpoints:
    """Resolve OAuth endpoints, zero-config by default: the host's honcho
    ``environment`` picks cloud vs localhost, a self-hosted ``base_url`` derives
    the token endpoint from the API host, and env vars override every field."""
...
def begin_authorization(
    ...
    *,
    ...
    ``source`` tags the initiating surface for consent branding; ``config_path``
    is the home-relative *display* string (the real write path goes to
    ``complete_authorization``)."""
    now = time.time() if now is None else now
    verifier, challenge = _pkce()
    state = secrets.token_urlsafe(32)
    with _pending_lock:
        for stale in [s for s, p in _pending.items() if now - p.created_at > _PENDING_TTL_SECONDS]:
...
def _install(
    ...
    *,
    ...
    """Persist ``grant`` for the target host; drop the cached client so the next
    acquisition uses the new token."""
    target_host = host or resolve_active_host()
    cred = oauth.install_grant(
        path or resolve_config_path(),
        target_host,
        grant,
...
def complete_authorization(
    ...
    *,
    ...
    ``apply_config=False`` stores tokens only (CLI path: settings stay wizard-owned)."""
    with _pending_lock:
        pending = _pending.pop(state, None)
    if pending is None:
        raise ValueError("unknown or expired authorization state")

    _status_code, grant = oauth._http_json(
        "POST", endpoints.token_url, timeout=oauth._REFRESH_TIMEOUT_SECONDS,
        data={"grant_type": "authorization_code", "client_id": endpoints.client_id, "code": code,
...
_CALLBACK_HTML = (
    "<!doctype html><meta charset=utf-8><title>Honcho connected</title>" + _CALLBACK_STYLE
    + "<div>Connected to Honcho. You can close this tab and return to Hermes.</div>"
).encode()
_CALLBACK_ERROR_HTML = (
    "<!doctype html><meta charset=utf-8><title>Honcho sign-in failed</title>" + _CALLBACK_STYLE
...
def _bind_loopback_server() -> tuple[HTTPServer, dict[str, str]]:
    ...
    Prefers :8765, else an OS-assigned port (the AS relaxes the port for loopback
    redirect URIs; the caller advertises the bound port)."""
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 - stdlib API name
            ...
            self.end_headers()
            if captured["error"]:
                self.wfile.write(_CALLBACK_ERROR_HTML.format(error=html.escape(captured["error"])).encode("utf-8"))
            else:
...
def capture_loopback_code(server: HTTPServer, captured: dict[str, str], *, timeout: float = 300.0) -> tuple[str, str]:
    """Serve ``/callback`` until our code lands; return ``(code, state)``.
    Loops so a stray probe to another path doesn't end the wait; raises
    ``TimeoutError`` if nothing arrives within ``timeout``."""
    ...
    if captured.get("error"):
        ...
        raise ValueError(f"authorization denied: {captured['error']}{f' ({detail})' if detail else ''}")
...
def authorize_via_loopback(
    *,
    ...
    """Full loopback flow: open browser → capture code → exchange → persist.
    ``open_url`` (default: system browser) always receives the authorize URL, so
    a CLI caller can print it for browserless setups."""
    ...
    return complete_authorization(
        endpoints, code, returned_state, config_path=path, host=host, apply_config=apply_config
    )
...
def supports_device_login(endpoints: OAuthEndpoints, *, timeout: float = 5.0) -> bool:
    ...
    Fails closed on any error / non-200 / missing capability."""
...
def request_device_code(endpoints: OAuthEndpoints, *, source: str | None = None) -> DeviceCode:
    ...
    if source:
        data["source"] = source
    status, body = oauth._http_post_form_status(endpoints.device_authorization_url, data, oauth._REFRESH_TIMEOUT_SECONDS)
    if status != 200:
    ...
    try:
        ...
        return DeviceCode(
            ...
            verification_uri_complete=body.get("verification_uri_complete", f"{verification_uri}?user_code={body['user_code']}"),
            expires_in=int(body["expires_in"]),
            interval=int(body.get("interval", 5)),  # §3.2: optional, default 5s
        )
...
def poll_for_token(
    ...
    *,
    ...
    """Poll the token endpoint until approved (RFC 8628 §3.4/§3.5).
    Sleeps ``interval`` before each poll, bumping it on ``slow_down``. Raises
    ``AccessDenied`` / ``DeviceCodeExpired`` on terminal outcomes and
    ...
    while True:
        ...
        try:
            status, body = oauth._http_post_form_status(
                endpoints.token_url,
                {"grant_type": DEVICE_GRANT_TYPE, "device_code": device.device_code, "client_id": endpoints.client_id},
...
def authorize_via_device_code(
    *,
    ...
    """Full device flow: request codes → show user code → poll → persist.
    ``open_url`` (if given) receives ``verification_uri_complete``; no default
    browser open, since the approving browser may be on another machine."""
    endpoints = resolve_endpoints()
    ...
    return _install(endpoints, grant, path=path, host=target_host, apply_config=apply_config, now=None, kind="device ")
...
def start_loopback_flow_background(
    *,
    ...
    global _flow_thread
    # Resolve under the caller's profile scope NOW — a context-local HERMES_HOME
    # override can't reach the worker thread.
