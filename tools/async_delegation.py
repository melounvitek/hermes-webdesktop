...
Backs ``delegate_task(background=true)``: the parent dispatches a subagent on
a module-level daemon executor and returns a handle immediately. When the
child finishes, a ``type="async_delegation"`` event is pushed onto the SHARED
``process_registry.completion_queue``, which the CLI and gateway already drain
while the agent is idle. Reusing that rail means completions surface as a NEW
turn (never spliced mid-turn, so role alternation and the prompt cache stay
intact) and inherit the queue's de-dup, crash-recovery and drain wiring.

The completion payload carries a self-contained task-source block (goal,
context, toolsets, model, dispatch time, status, result) so a parent deep in
unrelated context can use the result or re-dispatch.

This module owns ONLY the async lifecycle; the child build + run is injected
as a ``runner`` (``delegate_tool._run_single_child``).
"""
...
# Pending completions older than this are dropped on restart replay instead
# of re-run as a full-context turn; 48h keeps weekend results deliverable.
_MAX_COMPLETION_REPLAY_AGE_S = 48 * 3600.0
_DB_LOCK = threading.Lock()

# ── Stale-delegation detection (progress-based, on by default) ──────────────
# A runner wedged before returning (e.g. stuck in its first API call) never
# reaches its finalizer, so the delegation shows "dispatched" forever. We do
# NOT use a wall-clock timeout (legitimate heavy work must never be killed for
# taking long); a single monitor thread samples per-dispatch PROGRESS via an
# injected ``progress_fn``. Advancing children are left alone; a frozen one is
# interrupted, given a grace window to unwind through the normal finalize path,
# and only force-finalized with a terminal ``stalled`` event if it never returns.
# Thresholds mirror delegate_tool's sync heartbeat monitor: idle stays tight,
# in-tool is much higher so legitimately slow tools can finish.
...
def _copy_truthy(src: Dict[str, Any], dst: Dict[str, Any], keys) -> None:
    for k in keys:
        if src.get(k):
            dst[k] = src[k]


def _copy_present(src: Dict[str, Any], dst: Dict[str, Any], keys) -> None:
    for k in keys:
        if k in src:
            dst[k] = src[k]
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    # Preserve the journal mode SessionDB configured on state.db: forcing WAL
    # from every short-lived connection needs an exclusive lock and collides
    # with live transcript/FTS writers. The ledger works in WAL or DELETE mode.
    ...
    for name, sql_type in (
        ...
        ("delivery_claim", "TEXT"),
        ("delivery_claimed_at", "REAL"),
        # Raw api_server session id of the ORIGINATING request (wake target);
        # without it restart-recovered completions are unroutable there.
...
def _transaction() -> Iterator[sqlite3.Connection]:
    ...
    ``with conn:`` only commits/rolls back; without the explicit close every
    durable write leaks a connection (and WAL/SHM fds) until GC.
    """
    conn = _connect()
    try:
        with conn:
...
def _capture_routing_origin() -> Dict[str, Any]:
    """Snapshot scope_id/user_id/user_name from the dispatching turn.

    Captured on the PARENT thread (the daemon worker has no contextvars) and
    persisted so a restart-replayed completion can rebuild a SessionSource.
    ...
    try:
        ...
        for key in _ROUTING_KEYS:
            value = get_session_env(f"HERMES_SESSION_{key.upper()}", "")
            if value:
                origin[key] = value
...
def recover_abandoned_delegations() -> int:
    ...
    with _DB_LOCK, _transaction() as conn:
        ...
        for row in rows:
            ...
            _copy_truthy(task, event, _ROUTING_KEYS)
...
def restore_undelivered_completions(target_queue) -> int:
    ...
    Restored events are stamped ``restored=True`` in memory only: they came
    from a PREVIOUS process, so drains without an ownership filter must leave
    them for a consumer that can prove ownership. Rows older than
    ``_MAX_COMPLETION_REPLAY_AGE_S`` are terminally dropped instead of
    replaying a full-context turn nobody is waiting on.
    """
...
def release_completion_delivery(delegation_id: str, claim_id: str) -> bool:
    ...
    Attempts are counted at claim time; once the budget is exhausted the row
    converges to terminal ``dropped`` (only pending rows replay on restart).
    """
    now = time.time()
    with _DB_LOCK, _transaction() as conn:
        capped = conn.execute(
...
def drop_completion_delivery(delegation_id: str, claim_id: str) -> bool:
    ...
    Used when the spawning session ended at an explicit user boundary (/new,
    reset) rather than a compression rotation. ``dropped`` (not ``delivered``)
    keeps the ack honest; (not ``pending``)
    keeps restart recovery from replaying it into a fail-closed drop forever.
    """
    return _update_delivery(
        """UPDATE async_delegations SET delivery_state='dropped',
...
def get_durable_delegation(delegation_id: str) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "delivery_state": row[5], "delivery_attempts": row[6], "origin_session_id": row[7] or "",
    }


# ── In-memory registry queries ──────────────────────────────────────────────
def _get_executor(max_workers: int) -> ThreadPoolExecutor:
    """Lazily create (or grow, never shrink) the shared daemon executor.

    In-flight futures keep running on a replaced pool until it is collected.
    """
    global _executor, _executor_max_workers
    with _executor_lock:
        if _executor is None or max_workers > _executor_max_workers:
            _executor = DaemonThreadPoolExecutor(
                max_workers=max_workers, thread_name_prefix="async-delegate",
            )
...
def active_task_count() -> int:
    """Number of running child subagents (a batch of N contributes N).

    The truthful observability figure, unlike ``active_count()`` slots; a
    batch with no goal list falls back to 1.
    """
    def _size(r: Dict[str, Any]) -> int:
        goals = r.get("goals") if r.get("is_batch") else None
        return len(goals) if isinstance(goals, (list, tuple)) and goals else 1

    with _records_lock:
        return sum(_size(r) for r in _records.values() if r.get("status") in {"running", "finalizing"})


def _matches_session_selectors(
    record: Dict[str, Any], *, session_key: str = "", origin_ui_session_id: str = "",
    parent_session_id: str = "",
) -> bool:
    return any(
        wanted and str(record.get(field) or "") == wanted
        for field, wanted in (
...
def _session_records_locked(statuses, session_key, origin_ui_session_id, parent_session_id):
    """Records in ``statuses`` matching any selector. Caller holds ``_records_lock``."""
    return [
...
def has_live_for_session(
    session_key: str = "", origin_ui_session_id: str = "", parent_session_id: str = "",
) -> bool:
    """Whether a session still owns any live (running/stalling/finalizing) delegation."""
    if not session_key and not origin_ui_session_id and not parent_session_id:
        return False
    with _records_lock:
        return bool(_session_records_locked(_LIVE_STATES, session_key, origin_ui_session_id, parent_session_id))
...
def _current_origin_session_id() -> str:
    ...
    ``HERMES_SESSION_ID`` is unsafe here: building the child agent calls
    ``set_current_session_id(child.session_id)`` moments before dispatch, so
    the wake would self-post into the subagent's own session. The
    request-scoped ``HERMES_SESSION_CHAT_ID`` binding (raw X-Hermes-Session-Id
    on api_server) survives child construction; on push platforms chat_id is
    a chat, not a session, so yield ``""`` there.
    """
    try:
        from gateway.session_context import get_session_env
...
def _single_crash(error: str, duration: float) -> Dict[str, Any]:
    return {"status": "error", "summary": None, "error": error, "api_calls": 0,
...
def _dispatch(
    *, delegation_id: str, goal: str, goals: Optional[List[str]], context: Optional[str],
    ...
    parent_session_id: Optional[str], runner: Callable[[], Dict[str, Any]],
    ...
    Capacity check + record insert happen under ONE lock hold so concurrent
    dispatches can't both pass the check and exceed the cap. At capacity the
    dispatch is REJECTED (never queued) so a runaway model can't pile up
    ...
    record: Dict[str, Any] = {
        ...
        "goal": goal,
        **({"goals": list(goals)} if is_batch else {}),
        "context": context,
        ...
        **({"is_batch": True} if is_batch else {}),
...
def dispatch_async_delegation(
    *, goal: str, context: Optional[str], toolsets: Optional[List[str]], role: str,
    model: Optional[str], session_key: str, parent_session_id: Optional[str] = None,
    runner: Callable[[], Dict[str, Any]], origin_ui_session_id: str = "",
    origin_session_id: str = "", interrupt_fn: Optional[Callable[[], None]] = None,
    max_async_children: int = _DEFAULT_MAX_ASYNC_CHILDREN,
    progress_fn: Optional[Callable[[], tuple]] = None,
) -> Dict[str, Any]:
    """Spawn ``runner`` on the daemon executor and return a handle immediately.

    ``session_key``/``parent_session_id`` are captured on the parent thread
    (the worker carries no contextvars) and route the completion back to the
    spawning session. ``progress_fn() -> (token, in_tool)`` enables stale
    monitoring; omitted = unmonitored. Returns ``{"status": "dispatched",
    "delegation_id"}`` or ``{"status": "rejected", "error"}`` at capacity.
    """
...
def dispatch_async_delegation_batch(
    *, goals: List[str], context: Optional[str], toolsets: Optional[List[str]], role: str,
    ...
    origin_session_id: str = "", interrupt_fn: Optional[Callable[[], None]] = None,
    max_async_children: int = _DEFAULT_MAX_ASYNC_CHILDREN,
    delegation_id: Optional[str] = None, progress_fn: Optional[Callable[[], tuple]] = None,
) -> Dict[str, Any]:
    """Dispatch a WHOLE fan-out batch as ONE background unit.

    ``runner`` runs the entire batch and returns the combined
    ``{"results": [...], "total_duration_seconds": N}`` dict. The batch
    occupies ONE async slot (in-batch parallelism is bounded separately) and
    produces a SINGLE completion event carrying the per-task ``results``.
    """
...
def _finalize(delegation_id: str, result: Dict[str, Any], status: str) -> None:
    """Mark a record complete and push its completion event onto the queue."""
    event_record = _begin_finalization(delegation_id)
...
def _begin_finalization(delegation_id: str) -> Optional[Dict[str, Any]]:
    """Atomically claim terminal delivery; returns a record snapshot or None."""
    with _records_lock:
        ...
        # otherwise process shutdown can kill this daemon worker in the gap
        # after status flips but before SQLite is committed.
...
def _push_completion_event(record: Dict[str, Any], result: Dict[str, Any], status: str) -> None:
    ...
    Batch records (``is_batch``) carry the per-task ``results`` list instead of
    a single summary. Best-effort: failure must not crash the worker, but it
    WOULD mean a silently-lost result, so we log loudly.
    """
    ...
    if is_batch:
        # Batch payload: the full per-task results list (the formatter renders a
        # consolidated block) plus live transcript paths, which persist after
        # completion as the full-fidelity record of each child's run.
        payload = {
            "is_batch": True,
    ...
    evt = {
        ...
        **({"goals": record.get("goals")} if is_batch else {}),
    ...
    _copy_truthy(record, evt, _ROUTING_KEYS)
    _copy_present(result, evt, _STALL_META_KEYS)
...
def _ensure_stale_monitor() -> None:
    """Start (once) the stale-delegation monitor thread.

    One daemon thread serves every dispatch; it exits when no monitorable
    records remain and is restarted by the next dispatch with a ``progress_fn``.
    """
...
def _stale_monitor_loop() -> None:
    ...
    A changed progress token refreshes the record's timestamp. A frozen token
    past the idle/in-tool threshold marks the record ``stalling`` and calls
    ``interrupt_fn``; a ``stalling`` record still unreturned after the grace
    window is force-finalized with a terminal ``stalled`` event (a late
    runner return is then ignored by ``_begin_finalization``).
    """
    while not _monitor_stop.wait(_STALE_CHECK_INTERVAL):
        now = time.time()
        stalled: List[tuple] = []  # (delegation_id, quiet_for, in_tool)
        ...
        with _records_lock:
            for record in _records.values():
                ...
                if quiet_for >= limit:
                    ...
                    record.update(
                        status="stalling", _interrupted_at=now,
                        _stall_quiet_seconds=round(quiet_for, 2),
...
def _finalize_stalled(delegation_id: str) -> None:
    ...
    if event_record.get("is_batch"):
        result = {**_batch_crash(error, duration), **stall_meta}
    else:
        result = {**_single_crash(error, duration), "status": "stalled", "exit_reason": "stalled", **stall_meta}
    _push_completion_event(event_record, result, "stalled")
...
def _children_activity_from_token(token: Any, now: float) -> Optional[List]:
    ...
    delegate_tool emits one ``(api_call_count, current_tool, last_activity_ts)``
    tuple per child; foreign token shapes degrade to ``None`` entries.
    """
    try:
        parts = list(token)
    except TypeError:
...
def list_async_delegations() -> List[Dict[str, Any]]:
    ...
    Excludes callables and private monitor bookkeeping; adds computed live
    fields for UIs: ``seconds_since_progress``, ``children_activity`` /
    ``in_tool`` (sampled from ``progress_fn``), and stall context once the
    monitor has tripped.
    """
    now = time.time()
...
def _interrupt_records(targets: List[Dict[str, Any]], caller: str) -> int:
    """Call ``interrupt_fn`` on each record; returns how many succeeded."""
    count = 0
    for r in targets:
        ...
        if callable(fn):
            try:
                ...
                logger.debug("%s: %s interrupt failed: %s", caller, r.get("delegation_id"), exc)
    return count


def interrupt_all(reason: str = "shutdown") -> int:
    """Signal every running async delegation to stop (``/stop``, shutdown). Returns how many.

    The child still emits a completion event (status='interrupted') via the
    normal finalize path.
    """
    with _records_lock:
        targets = [r for r in _records.values() if r.get("status") in _ACTIVE_STATES]
    count = _interrupt_records(targets, "interrupt_all")
    if count:
...
def interrupt_for_session(
    ...
    """Signal running async delegations owned by ONE ending session to stop.

    Any matching selector claims the record: ``origin_ui_session_id`` (TUI
    tab), ``session_key`` (routing key at dispatch), or ``parent_session_id``
    (spawner's durable id — the right one for gateway chats, whose session_key
    survives ``/new`` while the session id rotates). Returns how many.
    """
    if not session_key and not origin_ui_session_id and not parent_session_id:
        return 0
    with _records_lock:
        targets = _session_records_locked(_ACTIVE_STATES, session_key, origin_ui_session_id, parent_session_id)
    count = _interrupt_records(targets, "interrupt_for_session")
    if count:
