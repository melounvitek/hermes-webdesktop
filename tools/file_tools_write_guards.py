...
def _check_binary_document_write(filepath: str, task_id: str = "default") -> str | None:
    ...
    if has_opaque_document_extension(filepath):
        ...
        return (
            ...
            "this document.")
    # Overwriting an existing binary (PDF, image, archive, SQLite db or its
    # -wal/-shm sidecar, ...) with text destroys it — the model only ever saw
    # extracted or mojibake text. Creating a NEW file with such an extension
    # stays allowed: raw PDF syntax is text-authorable and text fixtures
    # named ``*.db`` exist.
    if is_pdf_path(filepath) or has_binary_extension(filepath):
        try:
        ...
        try:
            if resolved.is_file():
                ...
                return (
                    ...
                    "(for SQLite databases and their -wal/-shm sidecars, the sqlite3 "
                    "CLI or a SQLite library). (Creating a NEW file with this "
