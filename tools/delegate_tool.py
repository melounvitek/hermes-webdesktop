...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "DO NOT USE FOR: a single tool call; mechanical multi-step work (use "
        "execute_code); tasks needing user interaction; or durable work that "
        "must survive this session (use cronjob or terminal(background=True, "
        "notify_on_complete=True) — /stop, /new, or process exit discards "
        "running subagents).\n\n"
        "RULES:\n"
        ...
        "verifiable handle (URL, ID, absolute path) and verify it yourself — "
        "fetch the URL, stat the file, read back the content — before "
        "reporting success.\n"
