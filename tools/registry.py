...
at import to declare schema, handler, toolset and availability check; ``model_tools.py``
queries it. Cycle-safe chain: this module imports nothing from model_tools or tool files;
tools/*.py import it; model_tools.py imports both; run_agent/cli import model_tools."""
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    sentinel) — one Browser session's live tools must not leak into another. A multiplex
    gateway installs a Hermes-home override per profile turn, so that key is the boundary."""
    try:
...
def _run_check_fn_uncached(fn: Callable, *, unresolved_scope: bool = False) -> bool:
    ...
    try:
        ...
        if unresolved_scope:
            # Expected fail-closed probe: boot-time check_fns run before any multiplex profile
            # secret scope exists. No traceback, so it isn't mistaken for a crashed check_fn.
            logger.debug(
...
def _check_fn_cached(fn: Callable) -> bool:
    ...
    with _check_fn_cache_lock:
        ...
        # No recent success — honor the failure; logged so silent tool loss is diagnosable.
        logger.warning(
...
class ToolRegistry:
    ...
    @staticmethod
    def _grouped(entries) -> Dict[str, List[ToolEntry]]:
    ...
    def register(
        ...
        with self._lock:
            ...
            # Availability is derived per-tool, so this map no longer gates a toolset; it still
            # feeds TOOLSET_REQUIREMENTS["check_fn"], which banner.py reads (presence only,
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        ``scope`` selects a profile overlay explicitly (multiplexed MCP tools live there);
        plugin callers may not name another scope, non-plugin callers default to the global
        map. Gated by the same opt-in as ``register(override=True)``, else a plugin could
        deregister a tool it doesn't own and re-register over the empty slot (the override
        check only runs when an entry exists). ``mcp-*`` is exempt — discovery repaves."""
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                if caller_owner is not None and not same_plugin \
