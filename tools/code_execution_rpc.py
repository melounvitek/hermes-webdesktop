...
def _rpc_server_loop(
    ...
    CURRENT cell's authority — the serving thread outlives many cells there.
    """
    if dispatch is None:
...
def _rpc_poll_loop(
    ...
    process, so this is safe alongside the script-execution thread. Malformed
    or unauthorized requests are removed without a response.
    """
    dispatch = _default_dispatch(task_id)
    poll_interval = 0.1
