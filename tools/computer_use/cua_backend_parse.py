...
/ ``get_window_state`` payload normalisation, key combos. No I/O, no module state — every
function depends only on its inputs, so the MCP and CLI transports share them safely.
"""
...
"""get_window_state AX-tree markdown line: ``[N] AXRole`` + label in one of four forms
(``= "value"``, ``"quoted"``, ``(paren)``, ``id=Label`` optionally after an ``(order)`` number).
...
def _action_result_from(name: str, ok: bool, message: str, meta: Dict[str, Any],
    ...
    """Build an ActionResult, lifting cua-driver's structured verdict. structuredContent is
    canonical, the flattened ``meta`` copy the fallback. Every structured field is additive:
    a driver that omits one leaves the attribute ``None`` so old drivers see unchanged behavior."""
...
def _parse_xprop_net_active_window(stdout: str) -> Optional[int]:
    ...
    0x...`` form, falling back to the first hex token."""
    text = stdout or ""
...
def _parse_elements_from_tree(markdown: str) -> List[UIElement]:
    ...
    drivers without ``structuredContent.elements``. Bounds are always ``(0, 0, 0, 0)`` (the
    markdown carries none), fine for element-index clicks since the driver resolves the frame."""
    return [
        UIElement(
            index=int(m.group(1)),
            role=m.group(2),
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    ...
    ``label`` and, when AT-SPI / AXFrame returned usable bounds, ``frame`` ``{x, y, w, h}`` —
    so real pixel bounds survive (the markdown path loses them). Malformed entries are skipped."""
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    ``image_mime_types`` is parallel to ``images`` with ``""`` where the part carried no
    mimeType (older drivers — callers then sniff the base64 prefix)."""
    data: Any = None
    images: List[str] = []
...
def _image_from_tool_result(out: Dict[str, Any]) -> tuple[Optional[str], Optional[str]]:
    ...
    (newer builds, CLI transport); checking both keeps capture() robust to the driver moving it."""
...
def _ingest_windows(raw_windows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    call needs integer ``pid`` and ``window_id``; on X11 the PID comes from the optional
    ``_NET_WM_PID`` property, so root/panel/popup windows report ``pid: null`` — skip those
    instead of aborting the enumeration. ``z_index``: higher = closer to front; Wayland's null
