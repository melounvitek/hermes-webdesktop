...
def _run_children_parallel(batch: _Batch, results: list, *, honor_parent_interrupt: bool) -> None:
    ...
    with DaemonThreadPoolExecutor(max_workers=batch.max_children) as executor:
        futures = {
            executor.submit(
                contextvars.copy_context().run, _run_single_child,
                task_index=i, goal=t["goal"], child=child, parent_agent=parent_agent, **batch.owner_kwargs(),
            ): i
        ...
        while pending:
            if honor_parent_interrupt and getattr(parent_agent, "_interrupt_requested", False) is True:
                ...
                for f in pending:
                    idx = futures[f]
                    results.append(
                        _future_entry(f, idx, _child_by_index.get(idx)) if f.done()
                        else _fabricated_entry(
...
def _execute_and_aggregate(batch: _Batch, *, honor_parent_interrupt: bool = True) -> dict:
    ...
    if len(batch.task_list) == 1:
        ...
        results.append(_run_single_child(_i, _t["goal"], child, batch.parent_agent, **batch.owner_kwargs()))
    else:
