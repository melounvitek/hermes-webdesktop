...
def rewrite_prompt_model_identity(agent, model: str, provider: str) -> None:
    """Rewrite the cached system prompt's ``Model:``/``Provider:`` lines after a provider switch.

    Not persisted to the session DB: the stored row keeps the primary's labels, so a
    restored primary replays a byte-identical prompt and its prefix cache still matches.
    Only the LAST occurrence of each line is touched — earlier matches may be user
    content (memory snapshots, context files).
    """
...
def _fallback_api_mode_hint(fb: dict, fb_provider: str, fb_base_url_hint: Optional[str]) -> tuple[bool, str]:
    ...
    resolve_provider_client() rewrites a dual-surface /anthropic base to /v1, losing the
    Anthropic wire signal, so detection runs on the URL the user configured. An explicit
    ``api_mode`` on the entry always wins — including "chat_completions" — and suppresses
    all later re-detection. ``provider: anthropic`` without a base_url uses the default
    endpoint and must still resolve to anthropic_messages.
    """
...
def _fallback_api_mode_resolved(agent, fb_provider: str, fb_model: str, fb_base_url: str) -> str:
    """Re-detect api_mode from provider / resolved base URL / model once the hint pass
    ...
    if fb_provider in {"nous", "nous-portal", "nousresearch"}:
        # Portal is dual-wire: anthropic/* must land on /v1/messages. resolve_provider_client
        # still returns an OpenAI client for Nous; the anthropic_messages swap rebuilds the native client.
        from hermes_cli.providers import nous_api_mode

        return nous_api_mode(fb_model)
    if _is_anthropic_wire_url(fb_base_url):
        # Named custom providers (e.g. cron-anthropic) resolve base_url from config,
        # so the hint pass never saw it.
    ...
    if agent._provider_model_requires_responses_api(fb_model, provider=fb_provider):
        # GPT-5.x usually needs Responses; provider exceptions (Copilot gpt-5-mini) stay inside the predicate.
        return "codex_responses"
...
def _rebind_fallback_credential_pool(agent, fb_provider: str, fb_model: str) -> None:
    """Rebind the credential pool when the provider changes: keeping the primary pool
    would let rate_limit/billing/auth recovery mutate the wrong credential set and
    overwrite the fallback's base_url. A same-provider pool (two openrouter entries)
    is preserved; otherwise the fallback provider's own pool is loaded so rotation works."""
...
_RATE_LIMIT_FAILOVER_REASONS = frozenset({
...
def _arm_rate_limit_cooldown(agent, reason: "FailoverReason | None") -> None:
    """Arm the primary's exponential rate-limit cooldown: 60s → 2m → 4m → ... → 4h cap.

    Only when leaving the primary: chain-switching from an already-active fallback means
    the primary was not the 429 source, so its cooldown is not reset/extended. Escalation
    only punishes providers that keep 429ing; the counter is reset by
    restore_primary_runtime on a successful restore.
    """
...
def _fallback_chain_exhausted(agent, reason: "FailoverReason | None") -> bool:
    """Chain exhausted: after walking a non-empty chain on a non-rate-limit failure
    (rate-limit reasons armed their own cooldown), arm a short cooldown so the next
    turn's restore_primary_runtime stays gated instead of replaying the whole context
    across every provider again. Always returns False."""
...
def _should_skip_fallback_candidate(agent, fb: dict, fb_key: tuple, fb_provider: str, fb_model: str, unavailable: set) -> bool:
    """True when the chain entry must be skipped (already unavailable, malformed,
    locally unusable, or the same backend that just failed)."""
    ...
    if local_skip_reason:
        ...
        return True
    # Falling back to the backend that just failed loops the failure. Identity semantics
    # (axes, shim aliases, credential surfaces, multi-endpoint pools) are owned by
    # agent.backend_identity — do not re-implement comparisons here.
...
def _swap_fallback_clients(agent, fb_client, fb_provider: str, fb_model: str, fb_base_url: str, fb_api_mode: str) -> None:
    """Install the fallback client(s) in place, honoring request_timeout_seconds
    (None = SDK default) the same way the primary client does."""
    ...
    # Preserve provider-specific headers resolve_provider_client() baked into fb_client
    # (the SDK stores default_headers in _custom_headers); otherwise later request-client
    # rebuilds drop them and providers requiring a User-Agent sentinel (Kimi Coding) 403.
    ...
    if timeout is not None:
        ...
        # Rebuild now so the timeout applies to the very next fallback request,
        # not only after a later credential-rotation rebuild.
        agent._replace_primary_openai_client(reason="fallback_timeout_apply")


def _update_fallback_context_compressor(agent) -> None:
    """Point compression limits at the fallback model's context window (not the
    primary's), respecting the explicit model.context_length config override."""
    compressor = getattr(agent, "context_compressor", None)
    if not compressor:
        return
    from agent.model_metadata import get_model_context_length

    # ``agent.api_key`` may be callable (Entra ID); the resolver's live probes need a string.
...
def _reresolve_fallback_reasoning_config(agent) -> None:
    """Per-model override > global reasoning_effort (YAML False = disabled); a config
    load failure keeps the current reasoning_config rather than killing the swap."""
    try:
...
def _rescope_fallback_extra_body(agent, old_model: str, old_provider: str, old_base_url: str) -> None:
    """Drop the OLD provider's custom_providers-contributed extra_body keys and merge
    the fallback provider's own.

    Removal is KEY-SCOPED: a key is dropped only if its value still equals what the old
    provider's config injected — a caller override of the same key won at init and
    differs, so it survives; keys the new provider redefines are re-added by the merge.
    """
    try:
...
def _buffer_fallback_notice(agent, notice: str) -> None:
    """Buffer the switch notice for terminal failure AND retain it as a durable one-shot
    for _emit_pending_fallback_notice (a successful fallback clears retry chatter)."""
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    """Switch to the next fallback model/provider in the chain; False when exhausted.

    Swaps client, model slug and provider in place so the retry loop continues on the
    new backend. Client construction goes through resolve_provider_client (no duplicated
    ...
    try:
        ...
        # Pass the entry's base_url/api_key so custom endpoints (e.g. Ollama Cloud) resolve
        # instead of falling through to OpenRouter defaults.
        ...
        # Clear the per-config context_length override so the fallback model's own
        # context window is resolved instead of the previous model's stale value.
        ...
        # Per-provider reasoning_content echo opt-in travels with the active provider;
        ...
        # Keep the prompt's self-identity in sync so "what model are you?" doesn't report the primary.
        ...
        # ``_fallback_activated`` is also reused by temporary `/model --once` restoration;
        # separate provenance so the restore path only emits a recovery notice after a real fallback.
...
# Message keys outside the Chat Completions schema that strict OpenAI-compatible gateways
# (Fireworks-backed OpenCode Go, Mistral, Moonshot/Kimi) reject with 422. The main loop
# drops these in ChatCompletionsTransport.convert_messages(); the summary path hand-builds
# messages and calls chat.completions.create() directly, so it must mirror that here:
# internal reasoning/finish_reason, tool_name (SQLite FTS bookkeeping), codex_* reasoning
# carriers, timestamp (kept on gateway user replay entries for stale-confirmation expiry),
# platform_message_id.
...
def _summary_sanitize_model(agent) -> str:
    """In MoA mode agent.model is the virtual preset; resolve the real aggregator model
    so Gemini preserves thought_signature."""
    if agent.provider == "moa":
        moa_client = getattr(agent, "client", None)
        agg_slot = getattr(moa_client, "last_aggregator_slot", None) if moa_client is not None else None
        if agg_slot and agg_slot.get("model"):
            return agg_slot["model"]
    return agent.model


def _iteration_summary_api_messages(agent, messages: list) -> list:
    """Build the wire-ready message list for the summary call, mirroring the main loop's
    api_messages build (sidecar substitution, tool-call repair, thinking-only drop,
    underscore-key sweep) since this path bypasses the transport."""
    needs_sanitize = agent._should_sanitize_tool_calls()
    sanitize_model = _summary_sanitize_model(agent) if needs_sanitize else None
    api_messages = []
    for msg in messages:
        api_msg = msg.copy()
        ...
        # api_content carries the exact bytes every main-loop call sent for this message;
        # substituting (not just popping) keeps the summary request's prefix identical so
        # the largest context isn't re-prefilled from the earliest sidecar-carrying message.
    ...
    # Compression/resume can orphan a tool result whose parent tool_call was summarized
    # away (Responses API: "No tool call found for function call output").
    api_messages = agent._sanitize_api_messages(api_messages)
    # Thinking-only assistant turns 400 on Anthropic-family providers; _thinking_prefill
    # must survive until here so the drop pass recognizes stubs after reasoning is stripped.
    api_messages = agent._drop_thinking_only_and_merge_users(api_messages)
    # Underscore-prefixed scaffolding keys: the transport's universal sweeper is bypassed here.
...
def _iteration_summary_chat_kwargs(agent, api_messages: list) -> dict:
    """chat.completions.create kwargs for the summary, mirroring
    ChatCompletionsTransport.build_kwargs() (this path bypasses the transport)."""
    try:
    ...
    if not is_lmstudio and agent._supports_reasoning_extra_body():
        extra_body["reasoning"] = (
            agent.reasoning_config if agent.reasoning_config is not None
    ...
    # Merge the profile's canonical body even when routing is unset: profiles may always
    # emit required metadata such as Portal tags.
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        # Strict machine-readable mode (hermes chat -Q, oneshot, background review): keep
        # diagnostics off stdout. quiet_mode is NOT the gate — the interactive CLI runs
        # quiet_mode=True by default and should still see this warning.
    ...
    # Shared constant so compaction recognizers can identify this runtime nudge by its
    # stable content after SessionDB projection strips metadata flags.
    ...
    try:
        ...
        # One retry on an empty summary; a summary that is empty once its <think> block
        # is stripped is NOT retried.
