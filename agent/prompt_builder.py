...
KANBAN_GUIDANCE = (
    ...
    "fields. If `kanban_show()` lists pre-created downstream review, "
    "QA, or release children that depend on your task, call `kanban_complete`: "
    "your implementation phase is done, and completion is what releases those "
    "children. Never sticky-block that parent for `review-required` and never "
    "request same-card review as well — either choice would strand or duplicate "
    "the downstream lane. Otherwise, when this same task needs review before it "
    "is final, call `kanban_request_review(summary=..., metadata=..., "
