...
1. :func:`generate_base_drafts` — a few cheap prompt-only "what should this pet
   look like" variants; the user picks one (or retries).
2. :func:`hatch_pet` — grounds one row strip per Hermes state on the chosen base,
   slices frames, composes + validates the atlas, and writes it to the store.

Splitting bounds cost (4 cheap base calls per round; the ~8 row calls happen
once, on the pet you keep) and gives each UI a natural preview/loading point.
"""
...
# Generations are independent network calls, fanned out so a hatch (~8 rows)
# doesn't blow the client's RPC timeout; capped to respect provider rate limits.
_MAX_PARALLEL_GENERATIONS = 4
# Row (re)generation attempts: early ones demand clean per-pose gutters, the
# last is lenient so a stubborn row still yields frames instead of dropping out.
_ROW_GEN_ATTEMPTS = 3
...
def _harden_transparency(path: Path) -> Path:
    ...
    ``background=transparent`` is requested but honored inconsistently, so run the
    row extractor's chroma-key pass on every base draft. Best-effort: a decode
    ...
    try:
        ...
        keyed = atlas._clear_transparent_rgb(keyed)  # no colored halo on the dark UI
        ...
        if out != path:
            # Nothing prunes cache/images outside the gateway loop: drop the
            # non-PNG original the hardened PNG replaces.
...
def _run_parallel(fn, items, *, cancelled, on_cancel_log: str) -> Iterator:
    ...
    ``as_completed`` runs on the caller's (request) thread, so callbacks fired
    from the results inherit the request's bound transport, unlike the workers.
    Once *cancelled* trips, queued work is cancelled and in-flight results dropped.
    """
...
def generate_base_drafts(
    ...
    *,
    ...
    Drafts run concurrently and are hardened to transparent cutouts;
    *on_draft(index, path)* fires as each finishes so UIs can stream previews.
    *is_cancelled* is polled cooperatively (unstarted drafts skipped, queued work
    cancelled, in-flight results dropped).
    """
    ...
    def _one(index: int) -> tuple[int, Path | None, str | None]:
        ...
        t0 = time.monotonic()
        # Distinct variation nudge per draft so the options aren't near-duplicates.
...
def hatch_pet(
    *,
    ...
    :class:`GenerationError` on failure. *is_cancelled* is polled cooperatively
    and, once tripped, aborts before composing/saving so a stopped hatch never
    writes a half-built pet.
    """
