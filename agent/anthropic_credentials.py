...
# platform.claude.com is the live token host; console.anthropic.com 404s but is kept as a
# fallback. _OAUTH_TOKEN_URL stays for backward-compatible imports.
_OAUTH_TOKEN_URLS = [
    "https://platform.claude.com/v1/oauth/token",
    "https://console.anthropic.com/v1/oauth/token",
]
_OAUTH_TOKEN_URL = _OAUTH_TOKEN_URLS[0]
# Anthropic 429s token-endpoint requests whose UA starts with ``claude-code/`` (or Mozilla);
# the real CLI uses bare axios there. Inference (build_anthropic_kwargs) still needs claude-code/.
...
class CredentialPersistError(RuntimeError):
    ...
    The refresh POST already spent the old refresh token, so a swallowed write failure
    leaves a consumed pair on disk that later replays as invalid_grant.
    """
...
def _atomic_write_private_json(path: Path, payload: Any) -> None:
    ...
    The token is never briefly umask-readable (write_text + chmod had a TOCTOU window); the
    random suffix avoids collisions with concurrent writers and crashed leftovers. The parent
    dir's mode is left alone (~/.claude/ is owned by Claude Code).
    """
...
# Fingerprints of secrets whose refresh POST succeeded but whose replacement never reached its
# store. Two scopes: process-local (OrderedDict) and a durable sidecar next to the shared
# singleton file so OTHER processes fail closed too. Non-reversible digests; never cleared.
...
def read_claude_code_credentials() -> Optional[Dict[str, Any]]:
    ...
    When both exist: prefer the only non-expired one (Claude Code 2.1.x refreshes one source
    but not the other), else the later ``expiresAt`` so a refresh uses the freshest
    refreshToken. ~/.claude.json primaryApiKey is deliberately excluded.
    """
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Refresh tokens are single-use and Claude Code refreshes on its own schedule, so we first
    re-read the live sources and adopt an already-rotated token instead of racing it into
    ``invalid_grant``. Read, decision, POST and write-back all run under the same path-keyed
    cross-process lock the pool uses; without it two profiles can still spend one refresh token.
    """
    try:
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout_seconds, target_path=cred_path):
            # Adopt only a DIFFERENT token with a real future expiry (0/absent expiresAt means
            # managed key / unknown expiry).
            ...
            if not refresh_token:
                ...
                return None
            # Another process may have spent this token and lost the commit; its sidecar
            # verdict is authoritative for the shared source.
            ...
            try:
                ...
                return None
            # The POST spent ``refresh_token``; this write is the commit step. On failure,
            # fail closed and mark the pre-rotation pair as spent.
            try:
...
def _write_claude_code_credentials(
    ...
    *scopes* (or the previously stored scopes) are persisted because Claude Code >=2.1.81 gates
    on ``"user:inference"`` being present. Raises ``CredentialPersistError`` on any failure,
    including a corrupt existing file.
    """
    cred_path = claude_code_credentials_path()
    try:
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Hermes historically persisted setup tokens into ANTHROPIC_TOKEN; that static token would
    otherwise win before the refreshable credential file is inspected, making refresh impossible.
    """
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    Enumerates with ``clear_expired=False, refresh=False`` (never ``select()``, which does both)
    so diagnostic call sites (account_usage, ``hermes models``) never mutate auth.json or hit
    the network; refresh-on-expiry belongs to the API call path's pool recovery.
    """
    try:
    ...
    for entry in entries:
        # access_token may be an explicit null on a persisted entry; a bare None.strip() would
        # escape the try/excepts and crash the resolver.
        token = (getattr(entry, "access_token", None) or "").strip()
        if getattr(entry, "auth_type", None) != AUTH_TYPE_OAUTH or not token:
            continue
        # load_pool() re-seeds rows from the singleton files, so a spent-but-uncommitted
        # rotation (possibly from another process) looks healthy here.
        ...
        if any(
            ...
            logger.debug(
                "Skipping Anthropic pool entry %s: rotated-but-uncommitted credential", getattr(entry, "id", "?")
            )
            continue
        return token
    return None
...
def run_oauth_setup_token() -> Optional[str]:
    ...
    # Interactive: stdin/stdout/stderr inherited so the user can complete the OAuth prompt;
    # the TUI-EOF stdin concern does not apply here.  noqa: subprocess-stdin
    try:
...
def _write_hermes_oauth_credentials(
    ...
    ``target`` lets a named profile commit a grant it BORROWED from the global root back to
    the ROOT singleton instead of forking a copy under its own HERMES_HOME; without this
    write-through the next ``load_pool()`` re-seeds the stale (consumed) pair from the file
    over the rotated pool entry. Raises ``CredentialPersistError`` when the pair does not land.
    """
