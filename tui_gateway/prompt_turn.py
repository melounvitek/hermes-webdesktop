...
def _plan_goal_compression_recovery(
    ...
    Compression exhaustion is a failed turn: never judge input, never a spent goal
    turn.  One fresh continuation is allowed; if that also exhausts, pause the goal
    instead of spinning until a random user message wakes it.  Returns
...
def _admit_prompt_turn(
    ...
    prompt.submit claims the slot in its RPC handler, but crash auto-continue,
    wake-ups and other synthesized turns call ``_run_prompt_submit`` directly —
    the bypass that once let a second backend run a duplicate turn.  Returns
...
def _record_turn_marker(session: dict, text: Any) -> str:
    ...
    Retired when the outcome reaches the client (_retire_turn_marker), so a
    surviving marker means the process died mid-turn and session.resume
    auto-continues from it.  Compression can rotate session_key mid-turn, so the
    caller keeps this key.  The key is published before the disk write so an
    interrupt racing startup can retire it; the post-write cancel check closes
    the inverse race where Stop lands first and clears no file yet.
    """
...
def _route_turn_images(agent, prompt: Any, images: list[str]) -> Any:
    ...
    try:
        ...
        print(
            f"[tui_gateway] native attach failed, falling back to text: {_img_exc}", file=sys.stderr,
        )
    return _build_image_ref_message(prompt, images)
...
def _start_turn_voice() -> tuple[Any, bool]:
    ...
    ``_tts_stream_begin`` goes first: it cuts any still-speaking previous turn,
    and that cut IS this turn's barge-in, so it must latch before the caller
    consumes the latch.  The full-duplex listener lets the user interject DURING
    generation (covers voice mode without working TTS).  The ambient "thinking"
    sound keeps long silent stretches from reading as a dead session; the
    per-blip gate skips while TTS audio flows or the mic is capturing; stopped in
    the turn's finally.
    """
...
def _apply_turn_notes(run_message: Any, session: dict) -> Any:
    ...
    Barge mid-speech -> tell the model so it can react to its own interruption;
    then reactions added since the last turn; then which window the message was
    typed into (HUD mode is per-turn state, so it cannot live in the byte-stable
    system prompt).
    """
...
class _TurnRun:
    ...
    ``agent`` is bound eagerly so the except/finally paths always have one even
    if setup throws; re-read after ``_sync_bot_capabilities``, which may swap in
    a rebuilt agent for Bot Chat sessions.  ``error_retained`` tells the finally
    to skip the normal inflight clear (a failed turn's snapshot stays for resume
    replay).  ``error_detail`` is the cause for the "tui turn finished" bookend,
    stashed by both failure paths because the finally sees neither ``result``
    nor the exception reliably; ``prompt_text`` is what was actually submitted
    (post @-expansion) so the cause can be checked for quoting it back.
    """
...
def _prepare_turn_input(sid: str, session: dict, st: _TurnRun, text: Any, images: list[str]):
    ...
    Returns ``(prompt, run_message, cols, streamer)`` or None when @-expansion was refused
    (error already emitted).  The config-model sync is skipped while a
    /model --once override is active: the once-model is deliberately not pinned
    as a session model_override, so the sync would clobber it back to the config
    model (a config.yaml change is adopted on the NEXT turn).  A model picked
    mid-turn was queued, not applied — apply it here, before the first model call
    and before the config sync so the explicit pick wins over a config change.
    """
...
def _invoke_agent(
    ...
    # Interim assistant text (commentary beside tool calls, or the attempted
    # final answer before a verify-on-stop nudge) is sealed by the desktop as
    # its own segment instead of being lost when message.complete replaces the
    # streaming buffer.  Gated on display.interim_assistant_messages (default true).
    ...
    try:
        ...
        # snapshot.  The join is deliberately unbounded — once stop is set it
        # only waits out one in-flight _get_usage/_emit, whose worst case (a
        # stalled transport write) would stall the message.complete emit just
        # the same.
        _usage_stop.set()
        _usage_thread.join()
...
def _log_turn_finished(sid: str, session: dict, st: _TurnRun, started_monotonic: float) -> None:
    ...
    accepted prompt always produces exactly one finished record.  agent.session_id
    is re-read because compression may have rotated it mid-turn (an
    accepted/finished pair whose id changed IS a rotation trace).  A missing
    finished record means the thread died before the finally."""
    result = st.result
    if isinstance(result, dict):
        status = (
            result.get("interrupted") and "interrupted" or result.get("error") and "error" or "complete"
        )
    else:
...
def _run_prompt_submit(
    ...
    def run():
        ...
        try:
            ...
            _invoke_agent(
                sid, session, st, prompt, run_message, streamer, images, display_kind, display_metadata,
            )
