"""Session flush / reaping / orphan sweep / cross-backend heartbeat: exit-flush signal handlers,
idle + LRU eviction, orphaned session-row sweep, backend heartbeat refresher.

Bodies are rebound onto server.py's globals at install time (method_ctx.bind_module), so they
reference server.py globals bare — including the knobs _SESSION_TTL_S, _REAPER_SCAN_S,
_EXIT_FLUSH_BUDGET_S and _INCREMENTAL_FLUSH_INTERVAL_S.
"""
...
def _flush_session_messages(session: dict | None) -> bool:
    """Best-effort durable flush of one session's transcript via ``agent._persist_session`` (same
    marker-deduped contract as ``_finalize_session``: repeated calls never duplicate rows)."""
...
def _flush_dirty_sessions(now: float | None = None) -> int:
    """Periodic incremental flush, driven by the idle-reaper scan. Skips ``running`` sessions: the
    turn thread owns mid-turn persistence and mutates the live message list, so racing it from the
    reaper thread is never safe. Idle sessions flush at most once per ``_INCREMENTAL_FLUSH_INTERVAL_S``;
...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    """Bounded flush of ALL in-memory sessions on the way out, on a daemon worker joined with the
    budget so a hung SQLite write can't block exit past ``HERMES_TUI_EXIT_FLUSH_BUDGET_S`` (default
    5s). Running sessions are included — the process is dying, a partial transcript beats loss."""
...
def _handle_exit_flush_signal(signum, frame) -> None:
    """Flush in-memory sessions, then hand off to the prior handler (uvicorn's graceful shutdown, a
    supervisor's handler, or the default disposition) — this only *prepends* a bounded flush."""
...
def install_exit_flush_signal_handlers() -> bool:
    """Install chaining SIGTERM/SIGINT flush handlers (main thread only). Called before uvicorn takes
    over signals: its ``capture_signals()`` saves these as the "original" handlers and re-raises into
    them after graceful shutdown, so the flush also covers terminations outside uvicorn's serve
    window. Idempotent; False off-main-thread/on failure."""
...
def _reaper_session_is_detached_idle(sid: str, session: dict) -> bool:
    """Shared hard exemptions for both reapers: never evict a session mid-turn, awaiting input, still
    building, owning live delegated work, or on a live transport. Lazy watch sessions never start a
    build, so their unset agent_ready must not make them immortal."""
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        ...
        with _sessions_lock:
            live = {
                lease.lease_id for session in _sessions.values()
...
# Soft LRU cap on in-memory sessions: the TTL reaper only frees sessions idle for hours, so a heavy
# reconnecting user accumulates resident detached agents. The cap evicts the least-recently-active
# DETACHED sessions sooner — never a running / pending / mid-build / live-transport one (reopening
# re-resumes from the DB). 0/null disables.
...
def _session_orphan_reaper_enabled() -> bool:
    """``dashboard.startup_orphan_sweep`` (default on). Fail-open on errors and on a missing key
    (raw yaml, no DEFAULT_CONFIG merge on this loader)."""
    try:
...
def _live_session_ids() -> list[str]:
    ...
    with _sessions_lock:
        for sid, session in _sessions.items():
            ...
            ids.update(str(c) for c in candidates if c)
    return sorted(ids)
...
def _sweep_orphaned_session_rows() -> list[str]:
    ...
    "Provably orphaned" is inferred conservatively: the row must have been created AND last messaged
    at least the session TTL ago (a fresh row that copied an old transcript is protected by its own
    ``started_at``). Rows held in memory (e.g. a ``session.resume`` in the startup grace window) are
    excluded. Cross-backend: the sweep refuses to close a row any live backend (heartbeat within
    ``2 * TTL``) could own — see ``SessionDB.sweep_orphaned_sessions``."""
    db = _get_db()
    if db is None or _SESSION_TTL_S <= 0:
        return []
    swept = db.sweep_orphaned_sessions(
        max_idle_seconds=_SESSION_TTL_S, sources=_ORPHAN_SWEEP_SOURCES, exclude_ids=tuple(_live_session_ids()))
    if swept:
        logger.info(
...
def _reaper_memo(fn, attr: str, compute):
    """Compute-once value memoised as an attribute on ``fn`` itself."""
...
def _backend_id_for_this_process() -> str:
    """Stable identity for this process's heartbeat row: pid (readability) AND a startup nonce so a
    PID-reuse respawn cannot inherit the dead predecessor's heartbeat."""
    import secrets as _secrets
    nonce = _reaper_memo(_backend_id_for_this_process, "_nonce", lambda: _secrets.token_hex(4))
...
def _gateway_started_at() -> float:
    """Wall-clock time this process started (first-call time is a good-enough proxy: the heartbeat
    refresher runs after the gateway is fully wired up)."""
    return _reaper_memo(_gateway_started_at, "_t", time.time)
...
def _start_backend_heartbeat_refresher() -> None:
    """Register this backend and start the refresher thread (once per process). The first refresh
    writes the row synchronously so this process's own sweep sees itself in the heartbeat table.
    ``_HEARTBEAT_REFRESH_S <= 0`` means "register once, never refresh"."""
...
def _schedule_startup_orphan_sweep() -> None:
    """Schedule the once-per-process startup orphan sweep, delayed by the WS-orphan grace window so
    a client reconnecting right after a restart can ``session.resume`` its row first. Grace 0 (park
    forever), TTL 0 and ``dashboard.startup_orphan_sweep: false`` all suppress the sweep."""
