...
``APIServerAdapter`` inherits it. api_server-internal helpers are imported lazily
inside each method (the origin imports this module, so a top-level import would be
a cycle), which also keeps ``patch("gateway.platforms.api_server.X")`` effective.
"""
...
async def _iter_stream_items(stream_q, agent_task, response):
    ...
    Woken directly by ``put_threadsafe`` (no executor hop / poll latency). Yields the
    ``None`` sentinel once so callers can run EOS-only work; when ``agent_task`` is
    already done the remaining queue is drained and the sentinel swallowed.
    """
...
def _trim_tool_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Trim large tool payloads in place so response.completed stays under ~100KB.

    Clients already received the full details via the incremental events.
    """
    for item in items:
...
class _ResponsesStream:
    ...
    Every event carries a monotonically increasing ``sequence_number`` (canonical
    Responses SSE schema). Text deltas are batched (50ms) to reduce Open WebUI
    re-render storms; tool events flush the buffer first.
    """
    ...
    def persist_incomplete_if_needed(self) -> None:
        """Persist an ``incomplete`` snapshot when no terminal one was written.

        Used by the disconnect and cancellation paths so GET /v1/responses/{id} and
        ``previous_response_id`` chaining keep working after abrupt termination.
        """
    ...
    async def emit_failed(self) -> None:
        redact = self._api._redact_api_error_text
        env = self.envelope("failed")
        env["output"] = self._final_items()
        env["error"] = {"message": redact(self.agent_error), "type": "server_error"}
        env["usage"] = self._api._responses_usage_payload(self.usage)
        history = self._history_with_user()
        history.append(
    ...
    async def emit_completed(self) -> None:
        ...
        # Compression-aware transcript substitution happens in _build_response_conversation_history
        # (result["_compressed"]); only a compression-rotated session_id is propagated here so
    ...
    async def emit_crash(self, exc: BaseException) -> None:
        env = self.envelope("failed")
        env["output"] = list(self.emitted_items)
        env["error"] = {"message": self._api._redact_api_error_text(exc, limit=500),
                        "type": "server_error"}
        env["usage"] = self._api._responses_usage_payload(self.usage)
        await self.write_event("response.failed", {"type": "response.failed", "response": env})
...
class OpenAICompatRoutesMixin:
    ...
    def _select_request_route(
        ...
        Returns ``(route, agent_overrides, error_response_or_None)``. A matching alias
        makes this request's agent use that route's model/provider instead of the
        global default.
        """
    ...
    def _spawn_stream_agent(self, stream_q, **run_kwargs) -> tuple:
        """Start ``_run_agent`` for an SSE writer; returns ``(agent_task, agent_ref)``.

        ``agent_ref[0]`` lets the writer interrupt on disconnect. The EOS sentinel is
        enqueued from the task's done callback so drain loops never race a polled
        ``agent_task.done()``.
        """
        def _on_delta(delta):
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # System messages become the ephemeral system prompt layered ON TOP of core; they are
        # flattened to text (Anthropic rejects images there, OpenAI text models ignore them).
        system_prompt = None
        ...
        if key_err is not None:
            return key_err
        # X-Hermes-Session-Id continues an existing session (history from state.db, not the
        # body). Requires a configured API key, otherwise any client could read arbitrary
        # history by guessing session ids.
        ...
        if provided_session_id:
            ...
            # Same entry-boundary guard as the native gateway (gateway.session._is_path_unsafe):
            # ids are interpolated into on-disk artifact filenames.
            ...
            # Stable id from the conversation fingerprint (system prompt + first user message are
            # constant across turns) so Open WebUI-style clients map onto one Hermes session.
            first_user = next(
        ...
        if stream:
            ...
            # tool_call_ids with an emitted "running" event: a "completed" without a matching
            # "running" (internal/filtered tools) is dropped rather than orphaned on the wire.
            _started_tool_call_ids: set[str] = set()

            def _on_tool_start(tool_call_id, function_name, function_args):
                """``hermes.tool.progress`` / ``status: running`` (legacy tool/emoji/label payload +
                toolCallId/status correlation). Names starting with ``_`` stay off the wire."""
            ...
            # tool_progress_callback is deliberately NOT wired: it fires alongside the structured
            # start/complete callbacks (which carry the tool_call id) and would duplicate emits.
        ...
        # Hard fail (no usable text AND a real failure) → 5xx with an OpenAI error envelope so
        # SDK clients raise instead of rendering the failure string as message.content.
    ...
    async def _write_sse_chat_completion(
        ...
        """Stream ``chat.completion.chunk`` frames from the agent's delta queue.

        On client disconnect the agent is interrupted (``agent.interrupt()``) so it
        stops making LLM calls, then the asyncio task wrapper is cancelled.
        """
        from gateway.platforms.api_server import (
        ...
        try:
            ...
            async for delta in _iter_stream_items(stream_q, agent_task, response):
                ...
                if isinstance(delta, tuple) and len(delta) == 2 and delta[0] == "__tool_progress__":
                    # Custom event so frontends can render tool lifecycle without the
                    # markers landing in conversation history.
                    await response.write(_sse_frame(delta[1], event="hermes.tool.progress"))
                else:
                    await response.write(_sse_frame(_chunk({"content": delta})))
            # The agent can fail after the queue drains cleanly (task raises, or result flagged
            # failed/partial); surface it as a non-"stop" finish_reason like the non-streaming path.
            ...
            # Agent crashed mid-stream: emit an error chunk so the client gets a proper
            # response instead of a TransferEncodingError from incomplete chunked encoding.
    ...
    async def _write_sse_responses(
        ...
        → ``response.completed`` (same envelope as non-streaming) or ``response.failed``.
        On disconnect the agent is interrupted and, when ``store=True``, an
        ``incomplete`` snapshot replaces the ``in_progress`` one so GET /
        ``previous_response_id`` chaining still work.
        """
        ...
        try:
            ...
            # Server-side cancellation (shutdown, request timeout): persist the incomplete
            # snapshot, then re-raise so the runtime's cancellation semantics are respected.
            st.persist_incomplete_if_needed()
            ...
            # Unhandled agent error (e.g. BadRequestError, AuthenticationError): emit
            # response.failed and terminate the stream cleanly so the client doesn't get a
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # Explicit conversation_history (stateless clients) takes precedence over
        ...
        # id. Binding the declared key is gated on that same precedence — a chain-selected
        # session must not have its routing key rewritten to this header.
        ...
        if stream:
            ...
            def _on_tool_progress(event_type, name, preview, args, **kwargs):
                # Structured start/complete callbacks carry the call id; progress events ignored.
                return
        ...
        # The effective session id from _run_agent carries compression-triggered rotations;
        # storing it keeps previous_response_id chaining off the pre-rotation session (which
        # would re-trigger compression on every request).
        ...
        # Output items come from the current turn only: AIAgent returns a full transcript in
        # result["messages"], older/mocked paths only the current-turn suffix.
    ...
    def _build_response_conversation_history(
        ...
        history prefix, so turn-start detection fails; concatenating the uncompressed
        history on front would bloat the stored context and re-trigger compression
        every request — the compressed transcript is stored as-is instead.
        """
        ...
        if isinstance(agent_messages, list) and agent_messages:
            turn_start = APIServerAdapter._response_messages_turn_start_index(
                conversation_history, user_message, result)
            # turn_start == 0: compression rewrote the transcript (use as-is) or
            # agent_messages is only the current turn.
    ...
    def _turn_transcript_messages(
        ...
        Clients accumulating ``assistant.delta`` events into one buffer cannot
        reconstruct intermediate assistant segments that preceded tool calls; the
        authoritative per-turn transcript on ``run.completed`` lets them reconcile
        without a separate ``GET /messages`` round-trip. Purely additive.
        """
        agent_messages = result.get("messages") if isinstance(result, dict) else None
        if not isinstance(agent_messages, list) or not agent_messages:
            return []
        ...
        for msg in agent_messages[start:]:
            if not isinstance(msg, dict) or msg.get("role") not in {"assistant", "tool"}:
                continue
            # _message_response projects compaction scaffolding itself and marks pure
            # handoffs display_kind == "hidden" (avoids re-running the content classifier).
