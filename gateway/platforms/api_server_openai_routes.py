...
class OpenAICompatRoutesMixin:
    ...
    async def _write_sse_responses(
        ...
        """
        from gateway.platforms.api_server import (
            _abandon_agent_task, _redact_api_error_text, _responses_usage_payload, _sse_frame)
        response = await self._prepare_sse_response(request, session_id, gateway_session_key)
        ...
        def _persist_response_snapshot(
            ...
            *,
            ...
            session_id_snapshot: Optional[str] = None) -> None:
            if not store:
                return
        ...
        def _persist_incomplete_if_needed() -> None:
            ...
            _persist_response_snapshot(
                incomplete_env, conversation_history_snapshot=incomplete_history)
        try:
            # response.created — initial envelope, status=in_progress
            created_env = _envelope("in_progress")
            created_env["output"] = []
            await _write_event("response.created", {
                "type": "response.created", "response": created_env})
            ...
            async def _emit_tool_completed(payload: Dict[str, Any]) -> None:
                ...
                emitted_items.append({
                    ...
                    "output": output_parts})
                await _write_event("response.output_item.added", {
                    "type": "response.output_item.added", "output_index": idx, "item": output_item})
                await _write_event("response.output_item.done", {
                    "type": "response.output_item.done", "output_index": idx, "item": output_item})
            ...
            if agent_error:
                ...
                _persist_response_snapshot(
                    failed_env, conversation_history_snapshot=_failed_history)
                terminal_snapshot_persisted = True
                await _write_event("response.failed", {
                    "type": "response.failed", "response": failed_env})
            else:
                completed_env = _envelope("completed")
                completed_env["output"] = final_items
                completed_env["usage"] = _responses_usage_payload(usage)
                full_history = self._build_response_conversation_history(
                    conversation_history, user_message, result, final_response_text)
                ...
                _persist_response_snapshot(
                    completed_env, conversation_history_snapshot=full_history,
                    session_id_snapshot=_result_sid if isinstance(_result_sid, str) and _result_sid else None,
                )
                terminal_snapshot_persisted = True
                await _write_event("response.completed", {
                    "type": "response.completed", "response": completed_env})
        except (ConnectionResetError, ConnectionAbortedError, BrokenPipeError, OSError):
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            return await self._write_sse_responses(
                request=request, response_id=response_id, model=model_name, created_at=created_at,
        ...
        async def _compute_response():
            return await self._run_agent(
                ...
                gateway_session_key=gateway_session_key,
                bind_declared_conversation=_declared_selected, **agent_overrides, route=route,
            )
        outcome, err = await self._run_idempotent(
