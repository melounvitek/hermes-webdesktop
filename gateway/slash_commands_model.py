...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    a first-time lazy import on a new code path can crash on a stale cached dependency. Scoped to
    model switching only (the highest-risk trigger).
    """
...
async def _persist_model_switch_to_config(result, config_path) -> None:
    ...
    Raw read is correct (merged defaults must not be persisted back). A scalar/None ``model:`` is
    coerced into a dict first or the assignments below raise ``TypeError``. Named providers
    re-resolve base_url/api_mode fresh, so leftovers are cleared; custom providers have no registry
    entry to re-derive from, so they need an explicit set-or-clear.
    """
...
class _ModelSwitchContext:
    ...
    def listing_kwargs(self) -> dict:
        return dict(
            current_provider=self.current_provider,
            current_base_url=self.current_base_url,
            current_model=self.current_model,
            user_providers=self.user_provs,
            custom_providers=self.custom_provs,
            excluded_providers=self.excluded_provs,
        )
...
async def _configured_display_context() -> tuple[dict, Optional[int]]:
    """(model config section, its ``context_length`` as int) for the switch confirmation; fail-open."""
    from gateway.run import _load_gateway_config
...
class GatewayModelCommandsMixin:
    ...
    async def _perform_model_switch(
        ...
        # Off the event loop: switch_model() can fall through to a synchronous models.dev HTTP
        # fetch (15s timeout) on a cold/expired cache, which would freeze the gateway.
    ...
    def _switch_cached_agent_model(self, result, ctx: _ModelSwitchContext, picker: bool) -> Optional[str]:
        ...
        The agent rolls back to the OLD working model/client and re-raises. Abort the commit (DB
        persist, session override, cache eviction, config write) so a failed switch is a no-op —
        otherwise the next message rebuilds a broken agent from the override.
        """
    ...
    async def _record_model_switch(
        ...
        # Session DB so the dashboard shows the updated model.
        _sess_db = getattr(self, "_session_db", None)
        if _sess_db is not None:
            try:
                _sess_entry = await self.async_session_store.get_or_create_session(source)
                # Typed path: consume an auto-reset flag so the next regular message's cleanup does
                # not wipe the override stored below.
        ...
        # Note prepended to the next user message (avoids system messages mid-history). Display
        # form strips opaque Palantir RID prefixes; the override map keeps the full ID for the wire.
        ...
        # re-resolved on rehydration). /model --once is EXCLUDED: a one-turn override must not
        # outlive a restart.
        if not one_turn:
            try:
        ...
        # Evict so the next turn builds a fresh agent from the override.
    ...
    async def _model_switch_confirmation(
        ...
        # Context: the provider-aware chain so Codex OAuth, Copilot and Nous-enforced caps win over
        # the raw models.dev entry.
        mi = result.model_info
        model_cfg, config_ctx = await _configured_display_context()
    ...
    async def _commit_model_switch(
        ...
        Shared by the typed ``/model <name>`` path and the picker callback (``picker=True``, which
        never carries a one-turn override).
        """
    ...
    async def _switch_and_commit(
        self, ctx: _ModelSwitchContext, model_id: str, provider_slug, *, source, picker: bool = False
    ) -> str:
        result, error = await self._perform_model_switch(ctx, model_id, provider_slug, source)
        if error is not None:
            return error
        return await self._commit_model_switch(result, ctx, source=source, picker=picker)
    ...
    async def _send_model_picker(self, event: MessageEvent, source, adapter, session_key: str, listing_kwargs: dict, on_model_selected) -> bool:
        ...
        *source* is the session-key-normalized source (Telegram topic recovery), so the picker's
        ...
        try:
            # Off-loop: provider listing can fall through to a synchronous HTTP fetch on a stale cache.
    ...
    async def _model_listing_reply(
        ...
        listing_kwargs = ctx.listing_kwargs()
        ...
        if adapter is not None and getattr(type(adapter), "send_model_picker", None) is not None:
            async def _on_model_selected(_chat_id: str, model_id: str, provider_slug: str) -> str:
                ...
                if profile_home is None:
                    return await self._switch_and_commit(ctx, model_id, provider_slug, source=event.source, picker=True)
        ...
        try:
            # Off-loop: provider listing can fall through to a stale-cache HTTP fetch.
    ...
    async def _model_selection_guard_reply(
        ...
        Runs the unified registry (cost + data-policy guards) off the event loop — pricing lookups
        may hit models.dev or a /models endpoint on a cache miss. Returns ``(fired, reply)``; the
        reply may be None when the platform rendered confirm buttons itself.
        """
        try:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        # --provider/--global/--session/--once/--refresh via the single-owner parser.
        ...
        if request.force_refresh:
            # Bust the disk cache so the picker shows live data.
            with contextlib.suppress(Exception):
                from hermes_cli.models import clear_provider_models_cache
                clear_provider_models_cache()

        # Normalize the source the same way a message turn does (Telegram DM topic recovery) before
        # deriving the override key, so the override is stored under the key the next turn reads.
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        # Persist the selection only — never agent.system_prompt (user-owned overlay). It lands in
        # get_hermes_home()/config.yaml (the routed profile under multiplex) and the next turn
        # re-resolves the prompt from it: no process-global state.
    ...
    def _save_gateway_config_key(self, key_path: str, value) -> bool:
        ...
        try:
            # Raw read: merged defaults must not be persisted back to the user's file.
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> Optional[str]:
        ...
        # Normalize the source (Telegram DM topic recovery) before deriving the override key so
        # storage matches the key the next message turn reads — same as /model.
        ...
        # The session's effective model (session /model override wins over config default) so
        # per-model reasoning_overrides display correctly.
        _session_model = str(
        ...
        # Interactive picker where supported (parity with /model); else the text status card.
    ...
    def _apply_fast_selection(self, session_key: str, value: str, persist: bool = False) -> str:
        ...
        # Session override; also the fallback when the config write failed so the user's choice
        # still applies (mirrors /reasoning --global).
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        # The /reasoning arg parser strips --global (any position) and normalizes unicode dashes.
        args, persist_global = self._parse_reasoning_command_args(raw_args)
