  ...
  settings: {
    ...
    notifications: {
      ...
      browser: {
        title: 'Hermes',
        body: '有對話需要你處理。',
        enable: '在此分頁啟用待處理事項通知',
        disable: '在此分頁停用',
        test: '傳送測試通知',
        description:
          '此分頁收到新的核准或輸入請求時，會傳送不含具體內容的提醒。不會分享任何對話文字。點擊提醒只會將焦點切換到此分頁。',
        limits:
          '僅支援桌面端，且僅在此分頁保持開啟並已連線時可用。目前的聊天取得焦點時，不會為該聊天傳送提醒。其他分頁和瀏覽器設定檔可能會收到不同的請求；請只在一個瀏覽器中啟用。重新載入、中斷連線、切換設定檔或暫停執行都會關閉提醒。不支援完成提醒、行動裝置提醒或關閉分頁後的提醒。瀏覽器和作業系統設定仍可能阻止通知顯示。',
        status: {
          off: '此分頁的提醒已關閉，即使瀏覽器已授予通知權限也不會傳送。',
          enabling: '正在等待通知權限及通知的獨佔存取權…',
          enabled: '已在此分頁啟用。同源的其他分頁無法同時啟用。',
          requested: '已請求傳送通知。如果未顯示，請檢查瀏覽器和系統的通知設定。',
          denied: '通知已遭封鎖。請先在瀏覽器的網站設定中變更權限，再重新啟用。',
          default: '未取得通知權限。提醒仍處於關閉狀態；你可以再次嘗試啟用。',
          insecure: '通知需要 HTTPS 或受信任的 localhost 環境。不支援使用一般 HTTP 的區域網路位址。',
          unsupported: '此瀏覽器不支援通知，或無法協調分頁以確保只有一個分頁使用通知。',
          disconnected: '連線無法使用，提醒已關閉。請重新連線後再次啟用。',
          otherTab: '另一個分頁正在處理通知。請先在該分頁停用，再於此處啟用。',
          failed: '通知請求失敗。提醒已關閉；請檢查瀏覽器和系統設定。',
          reset: '分頁或設定檔發生變更，提醒已關閉。準備好後請重新啟用。',
          limit: '已達通知記憶體上限。提醒已關閉；請重新載入此分頁後重試。'
        }
      },
      kinds: {
        approval: {
