  ...
  settings: {
    ...
    notifications: {
      ...
      browser: {
        title: 'Hermes',
        body: '確認が必要な会話があります。',
        enable: 'このタブで確認が必要なときの通知を有効にする',
        disable: 'このタブで無効にする',
        test: 'テスト通知を送信',
        description:
          'このタブが受信した新しい承認・入力リクエストを、内容を含まない通知でお知らせします。会話のテキストは共有されません。通知をクリックしても、このタブにフォーカスが移るだけです。',
        limits:
          'デスクトップのみ対応し、このタブが開いていて接続されている間だけ動作します。アクティブなチャットにフォーカスがある間、そのチャットの通知は表示されません。他のタブやブラウザープロファイルでは異なるリクエストを受信する場合があるため、有効にするブラウザーは1つだけにしてください。再読み込み、切断、プロファイルの変更、または一時停止で通知はオフになります。完了通知、モバイルでの通知、タブを閉じた状態での通知には対応していません。ブラウザーや OS の設定によっては通知が表示されない場合もあります。',
        status: {
          off: 'ブラウザーで通知が許可されていても、このタブではオフです。',
          enabling: '通知の許可と、通知をこのタブだけで扱うためのアクセス権を待っています…',
          enabled: 'このタブで有効です。同一オリジンの他のタブでは同時に有効にできません。',
          requested: '通知を要求しました。表示されなかった場合は、ブラウザーとシステムの通知設定を確認してください。',
          denied: '通知がブロックされています。ブラウザーのサイト設定で許可を変更してから、再度有効にしてください。',
          default: '通知が許可されませんでした。通知はオフのままです。もう一度有効にできます。',
          insecure:
            '通知には HTTPS または信頼できる localhost のコンテキストが必要です。通常の HTTP の LAN アドレスには対応していません。',
          unsupported: 'このブラウザーでは、通知または通知を1つのタブに限定するためのタブ間調整を利用できません。',
          disconnected: '接続できないため、通知はオフです。再接続してから、再度有効にしてください。',
          otherTab: '別のタブが通知を扱っています。そのタブで無効にしてから、ここで有効にしてください。',
          failed: '通知の要求に失敗しました。通知はオフです。ブラウザーとシステムの設定を確認してください。',
          reset:
            'タブまたはプロファイルが変更されたため、通知はオフになりました。準備ができたら再度有効にしてください。',
          limit: '通知用メモリの上限に達しました。通知はオフです。このタブを再読み込みしてやり直してください。'
        }
      },
      kinds: {
        approval: {
