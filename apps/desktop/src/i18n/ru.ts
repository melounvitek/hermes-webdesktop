...
export const ru = defineLocale({
  ...
  rightSidebar: {
    ...
    terminalReconnecting: 'Переподключение к терминалу…',
    terminalEmpty: 'В этом профиле нет терминалов',
    terminalErrors: {
      connection:
        'Не удалось подключиться к терминалу. Проверьте соединение и повторите попытку. Неотправленный ввод не будет отправлен повторно.',
      'missing-plugin':
        'Установите и включите плагин browser-terminal на сервере Hermes, затем перезапустите панель управления.',
      'missing-session': 'Этого терминала больше нет на сервере. Закройте вкладку и откройте новый терминал.',
      auth: 'Ошибка авторизации терминала. Войдите снова и повторите попытку.',
      superseded: 'Терминал подключён в другом месте. Повторите попытку, чтобы подключиться здесь.'
    },
