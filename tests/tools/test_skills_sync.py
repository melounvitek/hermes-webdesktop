...
class TestSyncSkills:
    ...
    def test_unmodified_skill_gets_updated_and_rebaselined(self, tmp_path):
        ...
        # The manifest records the new bundled hash so later changes are seen.
        assert manifest["old-skill"] == _dir_hash(bundled / "old-skill")
        assert manifest["old-skill"] != old_origin_hash

    def test_unchanged_skill_fast_path_skips_extra_work(self, tmp_path):
        """An unchanged bundled origin must not hash the (possibly
        bind-mounted) user copy, and rename recovery must not scan the active
        tree when every destination already exists."""
        bundled = self._setup_bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        user_skill = skills_dir / "old-skill"
    ...
    def test_fast_path_defers_user_hash_until_bundled_update(self, tmp_path):
        ...
        # A user-modified skill is never overwritten by a bundled update.
        assert "old-skill" not in changed.get("updated", [])
        assert (user_skill / "SKILL.md").read_text() == "# My local edit"

    def test_v1_manifest_migration_baselines_then_detects_updates(self, tmp_path):
        """v1 entries (no hash) baseline from the user's current copy; a later
        bundled change is then detected normally."""
        bundled = self._setup_bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"

        edited = skills_dir / "old-skill"
        edited.mkdir(parents=True)
        (edited / "SKILL.md").write_text("# Old modified by user")
        in_sync = skills_dir / "category" / "new-skill"
        in_sync.mkdir(parents=True)
        (in_sync / "SKILL.md").write_text("# New")
        (in_sync / "main.py").write_text("print(1)")

        manifest_file.write_text("old-skill\nnew-skill\n")  # v1 format

        with self._patches(bundled, skills_dir, manifest_file):
            migration = sync_skills(quiet=True)
            manifest = _read_manifest()
            edited_after_migration = (edited / "SKILL.md").read_text()
            # Upstream then ships a new version of the in-sync skill.
            (bundled / "category" / "new-skill" / "SKILL.md").write_text("# New v2")
            after = sync_skills(quiet=True)

        # The migration sync only records baselines; it touches nothing.
        assert "old-skill" not in migration.get("updated", [])
        assert "old-skill" not in migration.get("user_modified", [])
        assert edited_after_migration == "# Old modified by user"
        assert len(manifest["old-skill"]) == 32  # MD5 baseline recorded
        assert len(manifest["new-skill"]) == 32
        # With a baseline in place, the next bundled change is applied.
        assert "new-skill" in after["updated"]
        assert (in_sync / "SKILL.md").read_text() == "# New v2"

    def test_collision_with_user_skill_is_skipped_and_not_manifested(self, tmp_path):
        """A bundled skill whose name collides with an unmanifested user skill
        must be skipped without recording bundled_hash.
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            ...
            manifest = _read_manifest()
            resynced = sync_skills(quiet=True)  # second sync: must not flag
        ...
        assert "new-skill" not in resynced["user_modified"], (
    ...
    def test_backfills_official_optional_provenance(self, tmp_path):
        """Identical copies of official optional skills get hub provenance —
        including ones upstream later recategorized (the recorded install_path
        must be the ACTUAL location, else `repair-optional` can never find it).

        Copies that differ from upstream, or whose name is ambiguous in the
        active tree, are left alone: guessing would claim a user's skill as
        official.
        """
        bundled = self._setup_bundled(tmp_path)
        ...
        def _optional(rel, body):
            d = optional / rel
            d.mkdir(parents=True)
            (d / "SKILL.md").write_text(body)
            return d

        def _active(rel, body):
            d = skills_dir / rel
            d.mkdir(parents=True)
            (d / "SKILL.md").write_text(body)
            return d

        # (1) identical copy at the canonical path (with a nested reference).
        trl = _optional(
            "mlops/training/trl-fine-tuning", "---\nname: fine-tuning-with-trl\n---\n# TRL\n"
        )
        (trl / "references").mkdir()
        (trl / "references" / "api.md").write_text("api\n")
        active_trl = _active(
            "mlops/training/trl-fine-tuning", "---\nname: fine-tuning-with-trl\n---\n# TRL\n"
        )
        (active_trl / "references").mkdir()
        (active_trl / "references" / "api.md").write_text("api\n")

        # (2) identical copy still installed under the OLD category path.
        _optional("mlops/vector-databases/chroma", "---\nname: chroma\n---\n# Chroma\n")
        _active("mlops/chroma", "---\nname: chroma\n---\n# Chroma\n")

        # (3) locally edited copy — must not be claimed as official.
        _optional("category/edited-skill", "# upstream optional\n")
        _active("category/edited-skill", "# user modified\n")

        # (4) two installed dirs share a name — no basis to pick one.
        _optional("cat/dupe", "---\nname: dupe\n---\n# D\n")
        _active("x/dupe", "---\nname: dupe\n---\n# D\n")
        _active("y/dupe", "---\nname: dupe\n---\n# D\n")
        ...
        assert sorted(result["optional_provenance_backfilled"]) == [
            "chroma",
            "trl-fine-tuning",
        ]

        installed = json.loads((skills_dir / ".hub" / "lock.json").read_text())["installed"]
        entry = installed["trl-fine-tuning"]
        ...
        # The relocated skill records where it actually lives.
        assert installed["chroma"]["install_path"] == "mlops/chroma"

    def test_optional_backfill_avoids_redundant_scans_and_hashes(self, tmp_path):
        """Missing optional candidates share one active-tree index, and a skill
        that already has hub provenance is skipped before its (possibly
        bind-mounted) content is hashed."""
        bundled = self._setup_bundled(tmp_path)
        optional = tmp_path / "optional-skills"
        ...
        tracked_optional = optional / "category" / "tracked-skill"
        tracked_optional.mkdir(parents=True)
        (tracked_optional / "SKILL.md").write_text("# tracked\n")
    ...
    def test_repair_official_optional_restore_and_no_restore(self, tmp_path):
        """``--restore`` relocates a mangled copy to the canonical path (with a
        backup); without it, a modified canonical copy is left untouched and
        unclaimed."""
        bundled = self._setup_bundled(tmp_path)
        optional = tmp_path / "optional-skills"
        trl = optional / "mlops" / "training" / "trl-fine-tuning"
        trl.mkdir(parents=True)
        (trl / "SKILL.md").write_text(
            "---\nname: fine-tuning-with-trl\n---\n# Official TRL\n"
        )
        keep = optional / "mlops" / "training" / "keep-mine"
        keep.mkdir(parents=True)
        (keep / "SKILL.md").write_text("# official\n")
        ...
        modified = skills_dir / "mlops" / "training" / "keep-mine"
        modified.mkdir(parents=True)
        (modified / "SKILL.md").write_text("# modified\n")

        with self._patches(bundled, skills_dir, manifest_file):
            with patch("tools.skills_sync._get_optional_dir", return_value=optional):
                restored = restore_official_optional_skill(
                    "fine-tuning-with-trl", restore=True
                )
                untouched = restore_official_optional_skill("keep-mine", restore=False)

        canonical = skills_dir / "mlops" / "training" / "trl-fine-tuning"
        assert restored["ok"] is True
        assert restored["restored"] == ["trl-fine-tuning"]
        assert restored["backed_up"] == ["mlops/trl-fine-tuning"]
        assert "Official TRL" in (canonical / "SKILL.md").read_text()
        assert not wrong.exists()
        assert (Path(restored["backup_dir"]) / "mlops" / "trl-fine-tuning" / "SKILL.md").exists()

        installed = json.loads((skills_dir / ".hub" / "lock.json").read_text())["installed"]
        assert installed["trl-fine-tuning"]["source"] == "official"
        assert installed["trl-fine-tuning"]["install_path"] == "mlops/training/trl-fine-tuning"

        # Without --restore, the user's modified copy stays as-is and unclaimed.
        assert untouched["ok"] is True
        assert untouched["restored"] == []
        assert untouched["backfilled"] == []
        assert (modified / "SKILL.md").read_text() == "# modified\n"
        assert "keep-mine" not in installed
