...
close cannot cancel. Linux-only: the assertions read kernel truth from ``/proc``.
"""
...
def test_stray_in_process_close_does_not_let_a_sibling_unlink_the_wal(tmp_path, monkeypatch):
    pin_wal(monkeypatch)
    ...
    try:
        ...
        assert wal.exists() and wal.stat().st_ino == wal_inode, "sibling close unlinked the live WAL"
        db.append_message("s", role="user", content="after")  # would raise DeletedWalGenerationError
    finally:
        db.close()
    assert sqlite3.connect(db.db_path).execute("SELECT COUNT(*) FROM messages").fetchone()[0] == 2
