...
def test_parent_reexecs_tail_on_pulled_tree_and_relays_exit_code(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(update_handoff.subprocess, "run", fake_run)
    ...
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._hand_off_post_swap(
            ...
            _pre_update_plan=plan, _windows_gateway_resume=None)
    ...
    assert spawned["env"][update_handoff.POST_SWAP_ENV] == "1"
    assert spawned["env"][HANDOFF_PID_ENV] == str(os.getpid())
