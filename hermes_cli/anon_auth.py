...
# ``127.0.0.1`` while ``NOUS_INFERENCE_BASE_URL`` points at ``scripts/free_tier_fault_server.py``).
# Read from the environment, which the user controls, so it sits at the same trust level as the
# URL override itself; it never widens the NETWORK-side allowlist in ``auth_nous``.
