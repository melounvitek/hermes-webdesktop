...
The Python backing the install is different: it is shared by every Hermes profile because the
checkout's ``venv`` is shared. Runtime repair therefore uses an install-scoped store under
...
def managed_uv_path() -> Path:
    """Return the path where Hermes keeps *its* uv binary (``$HERMES_HOME/bin/uv[.exe]``).

    The directory may not exist yet -- callers should use ``ensure_uv()`` to bootstrap it.
    """
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    python-build-standalone binaries are ad-hoc signed, which leaves macOS TCC with a cdhash-only
    identity that changes whenever Hermes provisions a new runtime generation. An identifier-pinned
    designated requirement gives those generations a stable identity even when no Developer ID
    ...
    so an unavailable or incompatible ``codesign`` must not prevent the fixed interpreter from being
    installed.
    """
...
class _UvResult(str):
    ...
    POSIX only. This wrapper is **never** returned on Windows — see ``ensure_uv()`` for why the
    ``__iter__`` override is unsafe there.
    """
...
def _run_runtime_repair(
    uv_bin: str,
    repair_observer: Callable[[RuntimeRepairResult], None] | None,
    *,
...
def ensure_uv(
    *,
    ...
    On **POSIX** the result is a :class:`_UvResult` (a ``str`` subclass) that is both usable
    directly as the path *and* unpackable as ``(path, fresh_bootstrap)`` for older call sites parked
    on a 2-tuple release — see :class:`_UvResult` for the update-boundary rationale.

    On failure the result is falsy — never raises — so callers can fall back to pip gracefully.
    ``repair_observer``, when provided, receives the runtime repair result produced after a fresh uv
    bootstrap.
    """
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    self-update on every run is waste and, offline, an unbounded hang risk. A stamp file under
    HERMES_HOME caches the last successful self-update time.
    """
    try:
...
def update_managed_uv(
    *,
    ...
    Call this during ``hermes update`` so the managed copy stays current. Returns the managed path
    when uv is available and ``None`` otherwise. ``repair_observer``, when provided, receives the
    runtime repair result.

    The network self-update is skipped when it succeeded within the last
    ``UV_SELF_UPDATE_INTERVAL_SECONDS`` (7 days) unless ``force=True``; the vulnerable-runtime
    repair probe below ALWAYS runs — CVE-driven runtime repair must never be gated behind the
    freshness stamp.
    """
    existing = resolve_uv()
    if not existing:
    ...
    # what makes the migration happen on that first update. Runtime refresh is
    # deliberately non-fatal: the live venv was not touched unless a fully
    # prepared candidate reached cutover.
...
def _reload_hermes_constants():
    ...
    cannot import name 'venv_python_path' from 'hermes_constants' (~/.hermes/hermes-
    agent/hermes_constants.py)
    """
...
def _runtime_request(info: SQLiteRuntimeInfo) -> str:
    ...
    Requesting the exact patch can never repair some installs: for a given patch, python-build-
    standalone may have no artifact with fixed SQLite at all (e.g. every published 3.11.14 build
    links SQLite 3.50.4; the fix only exists from 3.11.15).
    """
...
def _list_available_patches(
    ...
    Returns [] on any failure (network, parse) -- callers fall back to the original bare-minor
...
def _attempt_install_generation(
    ...
    *,
    ...
    """One install+probe attempt for a specific version request (bare minor like "3.11", or an explicit
    patch like "3.11.15"). Each attempt gets its own generation directory so a rejected candidate's
    files are fully cleaned up before the next attempt, matching --reinstall semantics. Returns None
    (and cleans up) on any failure, including a vulnerable or off-line candidate.
    """
    ...
    install = subprocess.run(
        [uv_bin, "python", "install", request, "--reinstall", "--no-bin", "--no-registry", "--no-config"],
        **run,
    )
    ...
    if allow_minor_upgrade:
        ...
        if candidate.python_version < current.python_version:
            return reject(
                ...
                ".".join(str(p) for p in current.python_version), candidate.python_version,
            )
        ...
        return reject(
            ...
            ".".join(str(p) for p in current.python_version[:2]), candidate.python_version,
        )
...
def _retry_explicit_patches(
    ...
    *,
    ...
    allow_minor_upgrade: bool = False,
    skip_at_or_below: tuple[int, int, int] | None = None,
) -> tuple[Path, Path, SQLiteRuntimeInfo] | None:
    """Retry ``request``'s minor line with explicit patch versions, newest-first, at most
    ``_MAX_PATCH_RETRIES`` attempts, skipping versions already in ``tried`` (retrying one explicitly
    would spend a full download+install+probe+delete cycle to reach a certain rejection).

    ``skip_at_or_below`` also skips patches at or below that version: only NEWER patches can carry
    the SQLite fix, and the downgrade guard rejects the rest anyway. This matters on a uv whose
    download catalog is stale: in #71250 the newest indexed 3.11 was 3.11.14, exactly the installed
    version, so without this skip the loop burned all five retries walking backwards.
    """
...
def _install_safe_python_generation(
    ...
    *,
    ...
    result = _attempt_install_generation(uv_bin, request, tried_versions=tried_versions, **common)
    if result is not None:
        return result
    ...
    result = _retry_explicit_patches(
        uv_bin, request, tried=tried_versions,
        skip_at_or_below=current.python_version[:3], **common,
    )
    if result is not None:
        return result
    ...
    for next_minor in range(cur_minor + 1, 14):  # up to 3.13
        ...
        result = _attempt_install_generation(
            uv_bin, next_request, allow_minor_upgrade=True, tried_versions=fb_tried, **common
        )
        if result is not None:
            return result
        result = _retry_explicit_patches(
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    ``_detect_venv_python_processes`` excludes the calling process and its ancestors on purpose — a
    CLI ``hermes update`` itself runs from the venv python — which is correct for the dependency-
    sync path, where only a *loaded* ``.pyd`` image blocks the rewrite and a fresh child process
    dodges it.
    """
    if platform.system() != "Windows":
        return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    Only the Hermes-managed binary is ever refreshed; a caller-supplied foreign uv path is left
    alone.
    """
    managed = managed_uv_path()
    try:
        is_managed = Path(uv_bin).resolve() == managed.resolve()
    except OSError:
        is_managed = False
    if not is_managed:
        return False
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``venv`` wins when it holds an interpreter (managed layout takes precedence); otherwise fall
    back to ``.venv`` when that one does. When neither has an interpreter, return the ``venv`` path
    so the caller's existing ``not-applicable`` handling fires unchanged.
    """
    primary, fallback = root / _VENV_NAME, root / _ALT_VENV_NAME
    if not _venv_python(primary).is_file() and _venv_python(fallback).is_file():
        return fallback
    return primary
...
def _sweep_stale_runtime_backups(
    ...
    *,
    ...
    On POSIX, deleting the tree is safe even while an older process still maps files from it — open
    FDs and mmaps keep their inodes alive; the directory entry is what goes away.

    ``min_age_seconds`` guards against racing a concurrent repair in another process: a backup
    parked seconds ago may still be that repair's rollback path, so only clearly-old markers are
    swept. ``keep`` exempts the backup the current repair just created. Best-effort: never raises.
    """
    try:
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    def deferred(detail: str) -> RuntimeRepairResult:
        return RuntimeRepairResult(
            "skipped", detail, sqlite_before=current.sqlite_version_string
        )
...
def _install_uv(target: Path) -> None:
    ...
    (_install_uv_windows if system == "Windows" else _install_uv_posix)(env)
