...
(Accounts + API keys) **must be the same set**. Every provider added after those lists were written
silently went missing from the GUI — e.g.

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
:data:`hermes_cli.auth.PROVIDER_REGISTRY` (credential truth), and * ``display_name`` /
``description`` / ``signup_url`` from the provider's :class:`providers.base.ProviderProfile` when
one exists, falling back to the ``CANONICAL_PROVIDERS`` entry's ``label`` / ``tui_desc`` and the
``OPTIONAL_ENV_VARS`` signup URL otherwise (many profiles leave these blank, and four canonical
providers have no profile at all — lmstudio, openai-api, tencent-tokenhub, xai-oauth — so the
...
def _safe_import(module: str, attr: str, default):
    """Import ``attr`` from ``module``; return ``default`` on ANY failure.

    This module is on the import path of the web server and the CLI, and a
    provider-plugin import error must never blank the whole catalog.
    """
    try:
...
def provider_catalog() -> list[ProviderDescriptor]:
    ...
    Membership is :data:`CANONICAL_PROVIDERS` (auto-extended by provider plugins). Auth/env come
    from ``PROVIDER_REGISTRY``; display metadata from ``ProviderProfile`` with canonical/env
    fallbacks so providers without a profile still resolve sensibly.
    """
