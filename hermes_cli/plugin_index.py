...
Mirrors the Skills Hub catalog pattern (``tools/skills_hub.py``): a static machine-readable JSON
index hosted at a canonical URL, cached locally under ``HERMES_HOME/cache/`` with a TTL, with a
bundled seed file as the offline fallback and format reference.
...
def _parse_entries(raw: Any) -> List[PluginIndexEntry]:
    ...
    elif not isinstance(raw, list):  # bare-list form also accepted
...
def load_index(*, refresh: bool = False, offline: bool = False) -> tuple[List[PluginIndexEntry], str]:
    ...
    Returns ``(entries, source)`` where *source* is one of ``"remote"``, ``"cache"``, or ``"seed"``.

    Order: fresh cache (unless *refresh*) → remote → stale cache → bundled seed. ``offline=True``
...
def resolve_name(
    ...
    Returns ``(entry, candidates)``: an exact (case-insensitive) unique match in ``entry``,
    otherwise ``entry is None`` and ``candidates`` holds any partial matches (empty = nothing
    similar, >1 on exact = ambiguous).
    """
