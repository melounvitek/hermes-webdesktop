...
def _pip_install(
    ...
    *,
    ...
    # installed. ensure_uv() (not a pure lookup) because installing uv is in scope during setup, and
    # tier 2 is a pip that the Windows installer's `uv venv` does not seed.
...
# `/releases/latest` skips (it returned a package with zero binary assets and made every non-arm64
# host skip the install), and re-implementing upstream's tag resolution here would drift. Trust the
# installer: fresh installs run install.sh directly (it errors clean on a missing-arch asset);
...
def _confirmed_update_check(driver_cmd: str, require_confirmed_update: bool) -> tuple:
    """Ask the installed driver whether a newer release exists.

    Returns ``(proceed, pin_version)``. ``proceed=False`` means the caller should stop with
    success (already latest, or indeterminate under ``require_confirmed_update``). Best-effort: an
    older driver (no check-update verb) or an offline check yields None; then `hermes update`
    keeps the installed version — an indeterminate check must never cost a multi-minute silent
    reinstall on every update — while an explicit `computer-use install --upgrade` falls through.
    """
    _state = None
    try:
    ...
    if _state is not None and _state.get("update_available"):
        # Windows routine upgrades run unattended-safe rather than deferring: stdin is closed (a
        # consent Read-Host can't block), the version is pinned, the ceiling is
        # _CUA_BACKGROUND_UPDATE_TIMEOUT, and the installer preflights skip in seconds when the
        # lock is held or GitHub is unreachable. Only contract repairs and fresh installs stay
        # interactive-only — there upstream legitimately needs a human (autostart elevation,
        # SmartScreen).
        # Pin to the release check-update just confirmed: `latest_version` comes from the GitHub
        # Releases API so its assets exist, unlike the installer's baked version on `main`, which
        # is bumped before the assets are published and 404s when installed unpinned. Malformed
        # values are ignored → unpinned fallback.
        import re as _re
...
def install_cua_driver(
    ...
    way to upgrade. ``upgrade=False`` (toolset enable flow) keeps a compatible installation, repairs
    an old/incomplete one and installs when missing; ``upgrade=True`` always refreshes.
    """
...
# Ceiling for one upstream-installer run. Must exceed the installer's own stale-lock recovery
# window: _install-rust.sh serializes installs with ~/.cua-driver/packages/.install.lock.d and only
# force-releases a dead holder's lock after LOCK_STALE_AFTER_SECONDS=600. A shorter Python-side
# timeout kills every run before that recovery fires — a permanent "always times out" wedge.
# 660s = 600s lock window + 60s headroom for the download/swap.
_CUA_INSTALLER_TIMEOUT = 660

# Grace for draining the installer's pipes after a timeout kill. The kill is best-effort (see
# _reap_after_timeout), so the drain must be bounded: a surviving descendant still holds the
# inherited stdout handle and an unbounded read waits on an EOF that never comes, turning the
# ceiling above into no ceiling. A successful kill closes the pipe immediately, so this is free.
_CUA_INSTALLER_DRAIN_GRACE = 15

# Quiet unattended refreshes from ``hermes update``: bounded even when upstream waits on Read-Host
# or a consent prompt. Explicit ``computer-use install --upgrade`` keeps the full ceiling. (The
# lock/network preflights make a legitimate long wait impossible here, so a short ceiling is safe.)
...
def _cua_install_lock_held() -> bool:
    ...
    try:
        if sys.platform == "win32":
            ...
            # violation while held. The stale-clear already deleted an unheld file, so surviving
            # = held; confirm with an open probe.
            try:
...
def _kill_installer_tree(proc, *, is_windows: bool) -> None:
    ...
    try:
        if not is_windows:
            ...
            # PowerShell may leave download/install helpers alive after its direct process is
            # killed. They inherit stdout and can keep both communicate() and install.lock wedged,
            # so collect the tree first and kill it leaf-up.
...
def _reap_after_timeout(proc, *, is_windows: bool) -> None:
    ...
    An unbounded drain blocks on an EOF that only arrives when someone kills a surviving
    descendant by hand, so ``_CUA_INSTALLER_TIMEOUT`` would stop bounding anything.
    """
    _kill_installer_tree(proc, is_windows=is_windows)
    try:
        drained_out, _ = proc.communicate(timeout=_CUA_INSTALLER_DRAIN_GRACE)
        # The partial output names WHERE the installer was stuck (lock wait, consent prompt,
        # download); without it the timeout line is unactionable.
        if drained_out:
            logger.warning(
        ...
        # Deliberately not closing proc.stdout: communicate()'s reader threads are still blocked
        # on that handle and closing it underneath them races; they are daemon threads.
        logger.debug(
...
def _cua_installer_command(is_windows: bool):
    ...
    # path — avoiding both shell injection and a symlink/TOCTOU race on multi-user machines. The
    # manual hint stays the upstream one-liner the docs teach.
...
def _unattended_installer_preflight(install_cmd: list, is_windows: bool):
    ...
    if is_windows:
        ...
        # previous binary until the next interactive upgrade. Scriptblock invocation (instead of
        # `| iex`) is what lets us pass the parameter to a piped script.
        install_cmd = [
            "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass",
            "-Command",
            f"$sc = irm {_CUA_INSTALL_PS1_URL}; "
...
def _run_cua_driver_installer(
    ...
    already-installed system performs an upgrade. ``installer_timeout`` lets quiet callers use a
    shorter ceiling without weakening the explicit install path's stale-lock recovery window.
    """
    import platform as _plat
    ...
    # POSIX: own process group so a timeout kill takes out the whole `curl | bash` pipeline (and
    # the exec'd _install-rust.sh), not just the outer shell — surviving grandchildren would keep
    # holding the install lock and wedge every later run.
    popen_kwargs = {}
    if not is_windows:
        popen_kwargs["start_new_session"] = True

    try:
        # Non-verbose (`hermes update` refresh): capture the installer's chatty "Next steps" wall
        # and log it so a failure stays debuggable. Verbose interactive installs stream live.
        if verbose:
            ...
            # During `hermes update`, sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.hermes/logs/update.log — write straight to it so the full installer
            # output is kept (success AND failure) without echoing it to the terminal.
            if result.stdout:
