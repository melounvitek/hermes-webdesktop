...
def _git_ok(git_cmd, args, cwd, **kw) -> bool:
    ...
    try:
        return _git_run(git_cmd, args, cwd, **kw).returncode == 0
    except Exception:
        return False
    ...
    with suppress(OSError):
        ...
        if max_age_days > 0:
            ...
            for ref in refs:
                ...
                try:
                    ref_time = datetime.strptime(stamp, "%Y%m%d-%H%M%S").replace(tzinfo=timezone.utc)
                except ValueError:
                    continue
                if ref_time < cutoff:
                    stale.add(ref)
...
def _classify_fetch_failure(stderr: str) -> str:
    ...
    if _has_http_code("429") or "rate limit" in stderr.lower():
        return "✗ GitHub is rate limiting requests or having an outage (HTTP 429) — try again in 5 minutes."
    if _has_http_code("500", "502", "503", "504"):
        return "✗ GitHub appears to be having an outage — try again in a few minutes (https://www.githubstatus.com)."
...
def _locate_real_git() -> Optional[Path]:
    ...
    for candidate in candidates:
        # A failed probe (None) disqualifies the candidate just like a guard hit.
        if candidate.exists() and _probe_fork_bomb([str(candidate)]) is False:
            return candidate
    return None
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    def _dirty():
        out = _probe_run("diff", "-z", "--name-only")
        if out.returncode != 0:
            return None
    ...
    def _real_dirty():
        ...
        out = _probe_run("-c", "core.quotepath=false", "diff", "--numstat", "--ignore-cr-at-eol")
        if out.returncode != 0:
            return None
        # "<added>\t<deleted>\t<path>"; rename detection off, so exactly one path.
        return {
            parts[2]
            for parts in (line.split("\t", 2) for line in out.stdout.splitlines() if line.strip())
    ...
    with suppress(Exception):
        ...
        subprocess.run(
            git_cmd + ["config", "core.autocrlf", "false"], cwd=repo_root, capture_output=True, check=False,
        )
