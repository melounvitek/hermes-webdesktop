...
however they present command output. Keeping the logic here (not in cli.py / gateway/run.py) means
the two surfaces can never drift.

Subcommands: /suggestions list pending suggestions (numbered) /suggestions accept <N|id> create the
cron job for that suggestion /suggestions dismiss <N|id> dismiss it (latched, never re-offered)
/suggestions catalog seed the curated starter automations as pending /suggestions clear drop
accepted records (housekeeping)
"""
...
def _resolve_origin() -> Optional[Dict[str, Any]]:
    ...
    Mirrors cron's ``_origin_from_env`` so an accepted suggestion's job delivers back to the chat
    where it was accepted. Returns None if unavailable, in which case create_job falls back to a
...
def handle_suggestions_command(
    ...
    *,
    ...
    ``args`` is everything after ``/suggestions`` (already stripped of the command word). ``origin``
    is the platform/chat dict so an accepted job's "origin" delivery routes back to where the user
    accepted; when omitted it is resolved from the session environment.
    """
    if origin is None:
    ...
    if handler is None:
        return _USAGE
    return handler(store, rest, origin, surface)
