...
def scan_session_providers(
    ...
    call: Callable[[DashboardAuthProvider], object],
    *,
    phase: str,
    log: logging.Logger,
    ...
    registration order intact). ``swallow`` exceptions reject that candidate
    only. A ``ProviderError`` (IDP/JWKS unreachable) must NOT abort the chain —
    the credential may belong to a different, reachable provider; it is logged
    under ``phase`` and, if nothing else succeeds, re-raised as
    ``ProviderError(name)`` so the caller answers 503 instead of forcing a
    re-login through a possibly-unreachable path.
    """
