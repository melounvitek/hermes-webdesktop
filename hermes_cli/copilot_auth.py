...
def resolve_copilot_token() -> tuple[str, str]:
    ...
    Returns (token, source) where source describes where the token came from. Raises ValueError if
    only a classic PAT is available.
    """
...
def _try_gh_cli_token() -> Optional[str]:
    ...
    The result (including a miss) is cached for a short TTL — see the cache comment above. Callers
    that just re-authenticated can call ``_invalidate_gh_cli_token_cache()`` to re-probe
    immediately.
    """
    global _gh_cli_token_cache

    now = time.monotonic()
    if _gh_cli_token_cache is not None and now - _gh_cli_token_cache[0] < _GH_CLI_TOKEN_CACHE_TTL_SECONDS:
...
def _read_jwt_store(path: Path) -> Optional[dict]:
    ...
    Single chokepoint for every read of the persisted store (load, eviction, save-merge). A well-
    formed store is a few KB; a file over the 1 MiB cap or with non-dict content is treated as
    unusable so a corrupt/oversized file can't balloon memory or get rewritten back out.
    """
    if not path.exists():
        return None
    try:
...
def evict_cached_exchanged_token(raw_token: str) -> None:
    ...
    Used by the runtime stale-credential recovery path: when a live request starts failing with a
    Copilot ``model_not_available_for_integrator`` / ``model_not_supported`` 400, the cached
    exchanged token (or a degraded raw fallback that was cached in its place) is stale.
    """
    if not raw_token:
        return
...
def _load_jwt_from_disk(fp: str) -> Optional[tuple[str, float, Optional[str]]]:
    ...
    An oversized/corrupt store is treated as unusable — the caller re-exchanges (bound shared with
    eviction/save via _read_jwt_store).
    """
    path = _jwt_disk_path()
    if not path:
        return None
...
def exchange_copilot_token(raw_token: str, *, timeout: float = 10.0) -> tuple[str, float, Optional[str]]:
    ...
    The token is a semicolon-separated string (not a JWT) used as a Bearer token. ``base_url`` is
    the account-specific host: the exchange's ``endpoints.api`` (enterprise/proxied accounts), else
    derived from the token's ``proxy-ep``; individual accounts have neither, so it is None. Cached
    in-process until close to expiry. Raises ``ValueError`` on failure.
    """
    fp = _token_fingerprint(raw_token)
...
def _exchange_copilot_token_locked(
    ...
    # Re-check the in-process cache under the lock (a concurrent caller may have
    # just completed the exchange we were queued behind), then the on-disk
    # cache: a fresh process (e.g. gateway restart) has an empty in-process
    # cache but may have a still-valid persisted JWT. Reusing it avoids a
    # network round-trip at startup — precisely when the network is most likely
    # to be flaky and the single-shot exchange would degrade to the raw token.
...
def _derive_base_url_from_proxy_ep(token: str) -> Optional[str]:
    ...
    The token looks like ``tid=…;exp=…;proxy-ep=proxy.enterprise.githubcopilot.com;…``; the API host
    is that value with the leading ``proxy.`` replaced by ``api.``. None if proxy-ep is absent.
    """
    import re
...
def get_copilot_api_token(raw_token: str) -> tuple[str, Optional[str]]:
    ...
    Convenience wrapper: returns ``(api_token, base_url)`` on success, or ``(raw_token, None)`` if
    the exchange fails (e.g. network error, unsupported account type). This preserves existing
    behaviour for accounts that don't need exchange while enabling access to internal-only models
    for those that do.

    ``base_url`` is the account-specific API endpoint advertised by the exchange (``endpoints.api``,
    with a ``proxy-ep`` fallback), or None for individual accounts.
    """
    if not raw_token:
