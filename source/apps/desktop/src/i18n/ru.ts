...
export const ru = defineLocale({
  ...
  settings: {
    ...
    notifications: {
      ...
      browser: {
        title: 'Hermes',
        body: 'Одна из бесед требует вашего внимания.',
        enable: 'Включить уведомления о запросах, требующих внимания, в этой вкладке',
        disable: 'Отключить в этой вкладке',
        test: 'Отправить тестовое уведомление',
        description:
          'Общие оповещения о новых запросах на одобрение и ввод данных, полученных этой вкладкой. Текст бесед не передаётся. Нажатие на оповещение только переводит фокус на эту вкладку.',
        limits:
          'Только на компьютере, пока эта вкладка открыта и подключена. Для активного чата в фокусе оповещения не появляются. Другие вкладки и профили браузера могут получать другие запросы; включайте оповещения только в одном браузере. Перезагрузка, разрыв соединения, смена профиля или приостановка работы отключают оповещения. Оповещения о завершении, на мобильных устройствах и при закрытой вкладке не поддерживаются. Настройки браузера и ОС всё равно могут блокировать показ уведомлений.',
        status: {
          off: 'Отключено в этой вкладке, даже если разрешение браузера уже получено.',
          enabling: 'Ожидание разрешения и исключительного доступа к уведомлениям…',
          enabled:
            'Включено в этой вкладке. Другие вкладки с тем же источником (origin) не могут включить уведомления одновременно.',
          requested:
            'Показ уведомления запрошен. Если ничего не появилось, проверьте настройки уведомлений в браузере и системе.',
          denied:
            'Уведомления заблокированы. Измените разрешение в настройках сайта в браузере, прежде чем включать их снова.',
          default: 'Разрешение не получено. Оповещения остаются отключёнными; можно попробовать включить их снова.',
          insecure:
            'Для уведомлений нужен HTTPS или доверенный контекст localhost. Адреса локальной сети с обычным HTTP не поддерживаются.',
          unsupported:
            'В этом браузере недоступны уведомления или координация вкладок для исключительного доступа к ним.',
          disconnected:
            'Оповещения отключены, так как соединение недоступно. Подключитесь заново, затем включите их снова.',
          otherTab: 'Уведомлениями управляет другая вкладка. Отключите их там, прежде чем включать здесь.',
          failed: 'Не удалось запросить уведомление. Оповещения отключены; проверьте настройки браузера и системы.',
          reset: 'Оповещения отключены после изменения вкладки или профиля. Включите их снова, когда будете готовы.',
          limit:
            'Достигнут лимит памяти для уведомлений. Оповещения отключены; перезагрузите эту вкладку, чтобы начать заново.'
        }
      },
      kinds: {
        approval: {
  ...
  rightSidebar: {
    terminalEmpty: 'В этом профиле нет терминалов',
  ...
  preview: {
    ...
    saveToServer: 'Сохранить на сервере',
    serverSaved: 'Сохранено на сервере',
    browserInvalidText: 'Для редактирования нужен полный текстовый файл в UTF-8 размером не более 512 КБ.',
    browserOwnerChanged: 'Подключение или профиль изменились. Вернитесь к исходному файлу, чтобы сохранить правки.',
    browserDraftHint: 'При переключении файлов несохранённые правки остаются в этой вкладке браузера. Сохраните их перед перезагрузкой страницы или закрытием браузера.',
    browserConflict: 'Файл на сервере изменился. Черновик сохранён. Скопируйте нужные правки перед сбросом и перезагрузкой.',
    discardEditsTitle: 'Сбросить несохранённые правки?',
    discardEditsBody: 'Черновик будет потерян. Файл на сервере не изменится.',
    browserSaveHint: 'Редактируется файл на сервере. Автосохранения нет.',
    browserSaveWarning:
      'Сохранение заменяет файл на сервере и может сбросить права доступа и другие метаданные. Одновременные правки всё ещё могут быть перезаписаны. Только UTF-8; окончания строк могут измениться.',
    console: {
  ...
  desktop: {
    ...
    stopUnconfirmed: 'Не удалось подтвердить остановку предыдущего хода. Перед повторной отправкой попробуйте остановить его ещё раз или переподключитесь.',
