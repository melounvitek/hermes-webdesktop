...
export const zh = defineLocale({
  ...
  boot: {
    ...
    failure: {
      ...
      browserReloadHint: '重试会重新加载此页面。重新加载或离开页面登录会丢失未发送的附件。',
  ...
  settings: {
    ...
    notifications: {
      ...
      browser: {
        title: 'Hermes',
        body: '有对话需要你处理。',
        enable: '在此标签页启用待处理事项通知',
        disable: '在此标签页禁用',
        test: '发送测试通知',
        description:
          '此标签页收到新的批准或输入请求时，会发送不含具体内容的提醒。不会分享任何对话文本。点击提醒只会将焦点切换到此标签页。',
        limits:
          '仅支持桌面端，且仅在此标签页保持打开并已连接时可用。当前聊天获得焦点时，不会为该聊天发送提醒。其他标签页和浏览器配置文件可能会收到不同的请求；请只在一个浏览器中启用。重新加载、断开连接、切换配置文件或挂起都会关闭提醒。不支持完成提醒、移动端提醒或关闭标签页后的提醒。浏览器和操作系统设置仍可能阻止通知显示。',
        status: {
          off: '此标签页的提醒已关闭，即使浏览器已授予通知权限也不会发送。',
          enabling: '正在等待通知权限及通知的独占访问权…',
          enabled: '已在此标签页启用。同源的其他标签页无法同时启用。',
          requested: '已请求发送通知。如果未显示，请检查浏览器和系统的通知设置。',
          denied: '通知已被阻止。请先在浏览器的网站设置中更改权限，再重新启用。',
          default: '未获得通知权限。提醒仍处于关闭状态；你可以再次尝试启用。',
          insecure: '通知需要 HTTPS 或受信任的 localhost 环境。不支持使用普通 HTTP 的局域网地址。',
          unsupported: '此浏览器不支持通知，或无法协调标签页以确保只有一个标签页使用通知。',
          disconnected: '连接不可用，提醒已关闭。请重新连接后再次启用。',
          otherTab: '另一个标签页正在处理通知。请先在该标签页禁用，再在此处启用。',
          failed: '通知请求失败。提醒已关闭；请检查浏览器和系统设置。',
          reset: '标签页或配置文件发生变化，提醒已关闭。准备好后请重新启用。',
          limit: '已达到通知内存上限。提醒已关闭；请重新加载此标签页后重试。'
        }
      },
      kinds: {
        approval: {
