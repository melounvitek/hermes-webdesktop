#!/usr/bin/env bash
# Canonical test runner for the browser project. Run this instead of calling
...
# ── Locate python ───────────────────────────────────────────────────────────
# Plugin integration needs the external stock runtime's test interpreter;
# an explicit HERMES_PYTHON must take precedence over the lean installer venv.
#
...
if [ -n "$VENV" ]; then
  ...
  echo "▶ using explicit test interpreter via HERMES_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       or the explicit HERMES_PYTHON is not a python with pytest (create a test venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
# Forward only the knobs read by run_tests_parallel.py, never credential vars.
TEST_ENV=()
for _test_var in HERMES_TEST_WORKERS HERMES_TEST_PATHS \
