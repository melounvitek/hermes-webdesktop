...
# missing here reaches MATCH raw and raises (swallowed into zero results).
# re.escape so the backslash is not eaten. ``%`` is deliberately excluded: the
...
def _flatten_text(decoded: Any) -> str:
    """Multimodal part list -> joined text (or the placeholder); str passes
    through; anything else is ''."""
...
class SessionSearchMixin:
    ...
    def fts_rebuild_status(self) -> Optional[Dict[str, Any]]:
        ...
        "percent"}``, or None when no rebuild is pending. Reads via the pooled
        reader (not get_meta, which takes self._lock) so search_messages never
    ...
    def _rebuild_step(self, prefix: str, insert_sqls: List[str], *, fail_msg: str, finish) -> bool:
        ...
        def _do(conn):
            # Re-reading progress inside the BEGIN IMMEDIATE held by
            # _execute_write IS the claim: two workers cannot read the same value.
            ...
            if progress >= high_water:
                return False
            # Upper bound is an id, not a row count, so deleted-row gaps don't
            # shrink chunks below the claimed range.
    ...
    def _fts_teardown_trash_step(self) -> bool:
        ...
        Trash tables are PLAIN tables (their vtable parent was demoted), so no
        ...
        def _do(conn):
            ...
            if len(pk_cols) == 1 and (not pk_info or pk_info[0][1] == "INTEGER"):
                ...
                # disjoint ranges. Only INTEGER pks anchor the comparison (the
                # TEXT-pk config shadow table falls through below).
    ...
    def _fts_cjk_reset_if_stale(self) -> None:
        ...
        if self._execute_write(_do):
            # Recreate OUTSIDE the write transaction: _ensure_fts_cjk_schema uses
    ...
    def _fts_external_index_empty_with_messages(self, conn) -> bool:
        ...
        Healthy and mid-backfill installs never match. docsize is the
        authoritative "is this rowid indexed" surface for external-content
        FTS5; EXISTS not COUNT(*) because this runs on every writable open."""
        try:
    ...
    def _reseed_missing_progress(self, conn) -> None:
        """high_water without progress: fts_rebuild_step reads missing progress
        as "done by another process" and optimize would no-op then stamp.
        Reset a partially indexed DB to a known-empty surface (the chunk
        worker replays without an anti-join) and re-seed progress."""
    ...
    def _repair_optimize_bookkeeping(self) -> None:
        ...
        orphan high_water-without-progress gets progress re-seeded; an empty
        external index with messages and no markers (demote crash window /
        premature stamp) gets a full backfill claim. Never invents markers on a
        still-legacy inline DB — optimize would then skip demote and INSERT
        against the inline table forever."""
        def _do(conn):
    ...
    def _ensure_v23_fts_tables(self, failure_message: str) -> None:
        """Ensure the v23 base + trigram tables under the lock (IF NOT EXISTS,
        cheap); raise *failure_message* without the base table, since the
        backfill loop would otherwise retry "no such table" forever."""
        with self._lock:
    ...
    def optimize_fts_storage(
        ...
        if legacy and not pending:
            ...
            # Resume mid-demote: the process may have died between the staged
            # demote commit and schema ensure.
            self._ensure_v23_fts_tables("failed to re-create v23 messages_fts on optimize-storage resume")

        # A stale CJK index can only be recovered from scratch; then ensure
        # table + markers exist (a v23 DB gaining the cjk index for the first time).
        ...
        def _drive(phase: str, step) -> None:
            """Run *step* to completion; the inter-chunk sleep is the single
            place the duty cycle is enforced — back-to-back BEGIN IMMEDIATE
            chunks starve a live gateway/CLI out of its lock retries."""
            while True:
                _t0 = time.monotonic()
                if not step():
                    break
    ...
    def list_recent_user_messages(
        ...
        Bookkeeping timeline rows (``display_kind`` set) are excluded: no
        client counts them as user turns, and including them made ``/undo``
        soft-delete from a marker instead of the last real turn. Legacy
    ...
    def _sanitize_fts5_query(query: str) -> str:
        ...
        # 1. Protect balanced quoted phrases via numbered placeholders. Linear
        # scan, not regex, so pathological quote runs cannot backtrack.
        _quoted_parts: list = []
        pieces: list[str] = []
        i = 0
        while i < len(query):
        ...
        # 5. Quote dotted/hyphenated/underscored terms in ONE pass (sequential
        # passes double-quote ``my-app.config``).
    ...
    def _is_cjk_codepoint(cp: int) -> bool:
        return any(lo <= cp <= hi for lo, hi in SessionSearchMixin._CJK_RANGES)
    ...
    def _trigram_eligible_tokens(query: str) -> bool:
        ...
        tokens = [t for t in query.strip('"').strip().split() if t.upper() not in _FTS_OPERATORS]
    ...
    def _trigram_route_ok(self, raw_query: str) -> bool:
        """Per-token CJK length gate for the trigram index: ``广西 OR 桂林 OR
        漓江`` has 6 CJK chars total but 2 per token, so trigram returns 0."""
        return (
    ...
    def _match_rows(
        ...
        """Run one MATCH against *table*; ``None`` when the query cannot
        execute (tokenizer unavailable / syntax) so the caller falls back.

        *fail_open* names the index for the substring-capable routes: a
    ...
    def _search_messages_impl(
        ...
        if is_cjk:
            ...
            try:
                ...
                # Corruption parent class: detach the derived indexes and answer
                # from canonical rows; repair paths own the rebuild.
        ...
        # LIKE scan so old messages never vanish mid-rebuild. Cost decays to
        # zero as the backfill advances.
    ...
    def _search_cjk(self, query: str, wants_tool_rows: bool, route: Dict[str, Any]) -> List[Dict[str, Any]]:
        """CJK routing: the unicode61 table splits CJK into single characters
        ("大别山项目" -> "大 AND 别 AND ...": false positives, missed phrases).

        cjk-bigram serves every shape the legacy code split between trigram
        and LIKE, except role='tool' queries and LONE 1-char CJK runs (the
        ...
        if self._trigram_route_ok(raw_query) and not wants_tool_rows:
            ...
            if matches is not None:
                return matches
        non_op_tokens = [t for t in raw_query.split() if t.upper() not in _FTS_OPERATORS] or [raw_query]
        like_params: list = []
    ...
    def search_sessions_by_id(
        ...
        # compression chain) into SQL; over-fetch so the in-Python ranking has
        # candidates, then truncate.
    ...
    def _fts_table_exists(self, name: str) -> bool:
        """True if an FTS5 virtual table is queryable in this DB ("no such
        table" and "vtable constructor failed" — missing tokenizer /
        mid-teardown — both count as not queryable)."""
        try:
